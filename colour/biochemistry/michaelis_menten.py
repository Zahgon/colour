...
def reaction_rate_MichaelisMenten_Michaelis1913(
    ...
    *Michaelis-Menten* kinetics equation.
    ...
    V_max = xp_asarray(V_max, xp=xp, like=S)
    K_m = xp_asarray(K_m, xp=xp, like=S)
...
def reaction_rate_MichaelisMenten_Abebe2017(
    ...
    xp = array_namespace(S, V_max)

    V_max = xp_asarray(V_max, xp=xp, like=S)
    K_m = xp_asarray(K_m, xp=xp, like=S)
    b_m = xp_asarray(b_m, xp=xp, like=S)
...
def substrate_concentration_MichaelisMenten_Michaelis1913(
    ...
    *Michaelis-Menten* kinetics equation..
    ...
    V_max = xp_asarray(V_max, xp=xp, like=v)
    K_m = xp_asarray(K_m, xp=xp, like=v)
...
def substrate_concentration_MichaelisMenten_Abebe2017(
    ...
    *Michaelis-Menten* kinetics equation as specified by *Abebe, Pouli, Larabi
    ...
    xp = array_namespace(v, V_max)

    V_max = xp_asarray(V_max, xp=xp, like=v)
    K_m = xp_asarray(K_m, xp=xp, like=v)
    b_m = xp_asarray(b_m, xp=xp, like=v)
