...
def colour_fidelity_index_ANSIIESTM3018(
    ...
    # axis argument.
    averages_test = xp.matrix_transpose(
        xp_nanmean(
            xp_reshape(xp.matrix_transpose(bin_mask), (99, 1, 16), xp=xp)
            * xp_reshape(test_apbp, (*ref_apbp.shape, 1), xp=xp),
            axis=0,
            xp=xp,
        )
    )
    averages_reference = xp.matrix_transpose(
        xp_nanmean(
            xp_reshape(xp.matrix_transpose(bin_mask), (99, 1, 16), xp=xp)
    ...
    angles = (22.5 * xp.arange(16) + 11.25) / 180 * float(np.pi)
