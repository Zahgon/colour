...
def polynomial_expansion_Finlayson2015(
    ...
    existing_degrees = xp_asarray([1, 2, 3, 4], xp=xp)
...
def polynomial_expansion_Vandermonde(a: ArrayLike, degree: int = 1) -> NDArrayFloat:
    ...
    *Vandermonde* method.
    ...
    raveled = xp_reshape(a, (-1,), xp=xp)
    N = int(degree) + 1
    expanded = xp.matrix_transpose(
        xp.stack([raveled**i for i in range(N - 1, -1, -1)], axis=-1)
    )
    expanded = xp.concat(
        list(xp_reshape(expanded, (expanded.shape[0], -1, 3), xp=xp)), axis=1
    )

    result = expanded[:, 0 : expanded.shape[-1] - a.shape[-1] + 1]
    axes = tuple(i for i in range(result.ndim) if result.shape[i] == 1)
...
def apply_matrix_colour_correction_Cheung2004(
    ...
    RGB = as_float_array(RGB)
    CCM = as_float_array(CCM)

    xp = array_namespace(RGB)

    CCM = xp_asarray(CCM, xp=xp, like=RGB)
    shape = RGB.shape
...
def tps3d_parameters(
    ...
    *,
    ...
    # P: (N,4) -> [1, R, G, B]
    P = xp.concat([xp_asarray(ones((N, 1)), xp=xp, like=ctrl), ctrl], axis=1)
    ...
    Z = xp.zeros((4, 4))
    L = xp.concat([xp.concat([K, P], axis=1), xp.concat([P.T, Z], axis=1)], axis=0)

    V = xp.concat([dest, xp.zeros((4, 3))], axis=0)
...
def apply_tps3d(
    ...
    *,
    ...
    for start in range(0, M, chunk_size):
        ...
        P_all = xp.concat(
            [xp_asarray(ones((X.shape[0], 1)), xp=xp, like=X), X], axis=1
        )  # (m,4)
        ...
        chunks.append(U @ W + P_all @ A)

    out = xp.concat(chunks, axis=0)
