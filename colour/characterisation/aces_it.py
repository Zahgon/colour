...
def sd_to_aces_relative_exposure_values(
    ...
    **kwargs: Any,
    ...
    E_rgb = xp_asarray([E_r, E_g, E_b], xp=xp)
...
def training_data_sds_to_RGB(
    ...
    RGB = xp.matmul(
        xp.matrix_transpose(illuminant.values[..., None] * training_data.values),
...
def training_data_sds_to_XYZ(
    ...
    XYZ_w = xp.matmul(xp.matrix_transpose(cmfs.values), illuminant.values)
    XYZ_w *= 1 / XYZ_w[1]
...
def whitepoint_preserving_matrix(
    ...
    RGB_w = xp_asarray(RGB_w, xp=xp, like=M)
...
def camera_RGB_to_ACES2065_1(
    ...
    B = xp_asarray(B, xp=xp, like=RGB)
    b = xp_asarray(b, xp=xp, like=RGB)
    k = xp_asarray(k, xp=xp, like=RGB)
