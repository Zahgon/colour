...
from colour.utilities import xp_asarray
...
def test_colour_correction_tps3d_shape(xp: ModuleType) -> None:
    ...
    M_T = xp_asarray(rng.random((24, 3)), xp=xp)
    M_R = xp_asarray(rng.random((24, 3)), xp=xp)
    img = xp_asarray(rng.random((10, 11, 3)), xp=xp)

    out = colour_correction_TPS3D(img, M_T, M_R, smoothing=1e-8, chunk_size=1024)
    assert np.asarray(out).shape == (10, 11, 3)
