...
class TestIsWithinPointerGamut:
    ...
    def test_is_within_pointer_gamut(self, xp: ModuleType) -> None:
        ...
        assert is_within_pointer_gamut(xp_asarray([0.3205, 0.4131, 0.5100], xp=xp))

        assert not is_within_pointer_gamut(xp_asarray([0.0005, 0.0031, 0.0010], xp=xp))

        assert is_within_pointer_gamut(xp_asarray([0.4325, 0.3788, 0.1034], xp=xp))

        assert not is_within_pointer_gamut(xp_asarray([0.0025, 0.0088, 0.0340], xp=xp))
    ...
    def test_n_dimensional_is_within_pointer_gamut(self, xp: ModuleType) -> None:
        ...
        b = np.asarray(is_within_pointer_gamut(a))
