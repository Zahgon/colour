...
class TestRGB_to_IHLS:
    ...
    def test_RGB_to_IHLS(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            RGB_to_IHLS(xp_asarray([0.45620519, 0.03081071, 0.04091952], xp=xp)),
            np.array([6.26236117, 0.12197943, 0.42539448]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            RGB_to_IHLS(xp_asarray([0.00000000, 0.00000000, 0.00000000], xp=xp)),
            np.array([0.00000000, 0.00000000, 0.00000000]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            RGB_to_IHLS(xp_asarray([1.00000000, 1.00000000, 1.00000000], xp=xp)),
...
class TestIHLS_to_RGB:
    ...
    def test_IHLS_to_RGB(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            IHLS_to_RGB(xp_asarray([6.26236117, 0.12197943, 0.42539448], xp=xp)),
            np.array([0.45620519, 0.03081071, 0.04091952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            IHLS_to_RGB(xp_asarray([0.00000000, 0.00000000, 0.00000000], xp=xp)),
            np.array([0.00000000, 0.00000000, 0.00000000]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            IHLS_to_RGB(xp_asarray([0.00000000, 1.00000000, 0.00000000], xp=xp)),
    ...
    def test_n_dimensional_IHLS_to_RGB(self, xp: ModuleType) -> None:
        ...
        HYS = xp_asarray([6.26236117, 0.12197943, 0.42539448], xp=xp)
        RGB = np.asarray(IHLS_to_RGB(HYS))
