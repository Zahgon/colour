...
def normalised_primary_matrix(
    ...
    *primaries* and *whitepoint* :math:`xy` chromaticity coordinates.
    ...
    primaries = xp.matrix_transpose(xp.concat([primaries, z], axis=1))
...
def primaries_whitepoint(npm: ArrayLike) -> Tuple[NDArrayFloat, NDArrayFloat]:
    ...
    primaries = XYZ_to_xy(xp.matrix_transpose(xp.matmul(npm, xp.eye(3))))
    whitepoint = xp_reshape(
        XYZ_to_xy(
            xp.matrix_transpose(
                xp.matmul(npm, xp_asarray(ones((3, 1)), xp=xp, like=npm))
            )
        ),
        (-1,),
        xp=xp,
...
def RGB_luminance(
    ...
    Y = xp.sum(
        xp_asarray(
            normalised_primary_matrix(as_ndarray(primaries), as_ndarray(whitepoint))[1],
            xp=xp,
            like=RGB,
        )
        * RGB,
