...
def log_encoding_VLog(
    ...
    >>> np.round(log_encoding_VLog(L_in, 10, False) * 100).astype(np.int_)
    array([ 7, 42, 61])
    >>> np.round(log_encoding_VLog(L_in) * (2**10 - 1)).astype(np.int_)
    array([128, 433, 602])
    >>> np.round(log_encoding_VLog(L_in) * (2**12 - 1)).astype(np.int_)
    array([ 512, 1733, 2409])
