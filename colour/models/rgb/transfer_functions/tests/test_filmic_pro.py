...
log_encoding_FilmicPro6` definition.
        ...
        xp_assert_close(
            log_encoding_FilmicPro6(xp_asarray(0.0, xp=xp)),
            -np.inf,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FilmicPro6(xp_asarray(0.18, xp=xp)),
            0.606634519924703,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FilmicPro6(xp_asarray(1.0, xp=xp)),
            1.000000819999999,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_FilmicPro6` definition n-dimensional arrays support.
        ...
        x = 0.18
        y = np.asarray(log_encoding_FilmicPro6(xp_asarray(x, xp=xp)))
...
log_encoding_FilmicPro6` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_FilmicPro6(xp_asarray(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_FilmicPro6` definition.
        ...
        xp_assert_equal(log_decoding_FilmicPro6(xp_asarray(-np.inf, xp=xp)), 0.0)

        xp_assert_close(
            log_decoding_FilmicPro6(xp_asarray(0.606634519924703, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FilmicPro6(xp_asarray(1.000000819999999, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_FilmicPro6` definition n-dimensional arrays support.
        ...
        y = 0.606634519924703
        x = np.asarray(log_decoding_FilmicPro6(xp_asarray(y, xp=xp)))
...
log_decoding_FilmicPro6` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_FilmicPro6(xp_asarray(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
