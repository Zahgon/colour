...
log_encoding_AppleLogProfile` definition.
        ...
        xp_assert_close(
            log_encoding_AppleLogProfile(xp_asarray(0.0, xp=xp)),
            0.150476452300913,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_AppleLogProfile(xp_asarray(0.18, xp=xp)),
            0.488272458526868,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_AppleLogProfile(xp_asarray(1.0, xp=xp)),
            0.694552983055191,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_AppleLogProfile` definition n-dimensional arrays support.
        ...
        R = 0.18
        P = np.asarray(log_encoding_AppleLogProfile(xp_asarray(R, xp=xp)))

        R = xp.tile(xp_asarray(R, xp=xp), (6,))
        ...
        R = xp_reshape(xp_asarray(R, xp=xp), (2, 3), xp=xp)
        P = xp_reshape(xp_asarray(P, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            log_encoding_AppleLogProfile(R),
            P,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        R = xp_reshape(xp_asarray(R, xp=xp), (2, 3, 1), xp=xp)
...
log_encoding_AppleLogProfile` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_AppleLogProfile(xp_asarray(R * factor, xp=xp)),
                    P * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_AppleLogProfile` definition.
        ...
        xp_assert_close(
            log_decoding_AppleLogProfile(xp_asarray(0.150476452300913, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_AppleLogProfile(xp_asarray(0.488272458526868, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_AppleLogProfile(xp_asarray(0.694552983055191, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_AppleLogProfile` definition n-dimensional arrays support.
        ...
        P = 0.398764556189331
        R = np.asarray(log_decoding_AppleLogProfile(xp_asarray(P, xp=xp)))
...
log_decoding_AppleLogProfile` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_AppleLogProfile(xp_asarray(P * factor, xp=xp)),
                    R * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
