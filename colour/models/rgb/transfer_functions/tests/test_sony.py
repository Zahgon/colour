...
log_encoding_SLog` definition.
        ...
        xp_assert_close(
            log_encoding_SLog(xp_asarray(0.0, xp=xp)),
            0.088251291513446,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog(xp_asarray(0.18, xp=xp)),
            0.384970815928670,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog(xp_asarray(0.18, xp=xp), 12),
            0.384688786026891,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog(xp_asarray(0.18, xp=xp), 10, False),
            0.376512722254600,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog(xp_asarray(0.18, xp=xp), 10, False, False),
            0.359987846422154,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog(xp_asarray(1.0, xp=xp)),
            0.638551684622532,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_SLog` definition n-dimensional arrays support.
        ...
        x = 0.18
        y = np.asarray(log_encoding_SLog(xp_asarray(x, xp=xp)))
...
log_encoding_SLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_SLog(xp_asarray(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_SLog` definition.
        ...
        xp_assert_close(
            log_decoding_SLog(xp_asarray(0.088251291513446, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog(xp_asarray(0.384970815928670, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog(xp_asarray(0.384688786026891, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog(xp_asarray(0.376512722254600, xp=xp), 10, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog(xp_asarray(0.359987846422154, xp=xp), 10, False, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog(xp_asarray(0.638551684622532, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_SLog` definition n-dimensional arrays support.
        ...
        y = 0.384970815928670
        x = np.asarray(log_decoding_SLog(xp_asarray(y, xp=xp)))
...
log_decoding_SLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_SLog(xp_asarray(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_encoding_SLog2` definition.
        ...
        xp_assert_close(
            log_encoding_SLog2(xp_asarray(0.0, xp=xp)),
            0.088251291513446,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog2(xp_asarray(0.18, xp=xp)),
            0.339532524633774,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog2(xp_asarray(0.18, xp=xp), 12),
            0.339283782857486,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog2(xp_asarray(0.18, xp=xp), 10, False),
            0.323449512215013,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog2(xp_asarray(0.18, xp=xp), 10, False, False),
            0.307980741258647,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog2(xp_asarray(1.0, xp=xp)),
            0.585091059564112,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_SLog2` definition n-dimensional arrays support.
        ...
        x = 0.18
        y = np.asarray(log_encoding_SLog2(xp_asarray(x, xp=xp)))
...
log_encoding_SLog2` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_SLog2(xp_asarray(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_SLog2` definition.
        ...
        xp_assert_close(
            log_decoding_SLog2(xp_asarray(0.088251291513446, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog2(xp_asarray(0.339532524633774, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog2(xp_asarray(0.339283782857486, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog2(xp_asarray(0.323449512215013, xp=xp), 10, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog2(xp_asarray(0.307980741258647, xp=xp), 10, False, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog2(xp_asarray(0.585091059564112, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_SLog2` definition n-dimensional arrays support.
        ...
        y = 0.339532524633774
        x = np.asarray(log_decoding_SLog2(xp_asarray(y, xp=xp)))
...
log_decoding_SLog2` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_SLog2(xp_asarray(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_encoding_SLog3` definition.
        ...
        xp_assert_close(
            log_encoding_SLog3(xp_asarray(0.0, xp=xp)),
            0.092864125122190,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog3(xp_asarray(0.18, xp=xp)),
            0.41055718475073,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog3(xp_asarray(0.18, xp=xp), 12),
            0.410557184750733,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog3(xp_asarray(0.18, xp=xp), 10, False),
            0.406392694063927,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog3(xp_asarray(0.18, xp=xp), 10, False, False),
            0.393489294768447,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_SLog3(xp_asarray(1.0, xp=xp)),
            0.596027343690123,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_SLog3` definition n-dimensional arrays support.
        ...
        x = 0.18
        y = np.asarray(log_encoding_SLog3(xp_asarray(x, xp=xp)))
...
log_encoding_SLog3` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_SLog3(xp_asarray(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_SLog3` definition.
        ...
        xp_assert_close(
            log_decoding_SLog3(xp_asarray(0.092864125122190, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog3(xp_asarray(0.41055718475073, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog3(xp_asarray(0.410557184750733, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog3(xp_asarray(0.406392694063927, xp=xp), 10, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog3(xp_asarray(0.393489294768447, xp=xp), 10, False, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_SLog3(xp_asarray(0.596027343690123, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_SLog3` definition n-dimensional arrays support.
        ...
        y = 0.41055718475073
        x = np.asarray(log_decoding_SLog3(xp_asarray(y, xp=xp)))
...
log_decoding_SLog3` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_SLog3(xp_asarray(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
