...
dcdm.eotf_inverse_DCDM` definition.
        ...
        xp_assert_close(
            eotf_inverse_DCDM(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_DCDM(xp_asarray(0.18, xp=xp)),
            0.11281861,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_DCDM(xp_asarray(1.0, xp=xp)),
            0.21817973,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        assert (
            np.asarray(eotf_inverse_DCDM(xp_asarray(0.18, xp=xp), out_int=True)) == 462
        )
...
eotf_inverse_DCDM` definition n-dimensional arrays support.
        ...
        XYZ = 0.18
        XYZ_p = np.asarray(eotf_inverse_DCDM(xp_asarray(XYZ, xp=xp)))

        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6,))
        XYZ_p = xp.tile(xp_asarray(XYZ_p, xp=xp), (6,))
        xp_assert_close(eotf_inverse_DCDM(XYZ), XYZ_p, atol=TOLERANCE_ABSOLUTE_TESTS)

        XYZ = xp_reshape(xp_asarray(XYZ, xp=xp), (2, 3), xp=xp)
        XYZ_p = xp_reshape(xp_asarray(XYZ_p, xp=xp), (2, 3), xp=xp)
        xp_assert_close(eotf_inverse_DCDM(XYZ), XYZ_p, atol=TOLERANCE_ABSOLUTE_TESTS)

        XYZ = xp_reshape(xp_asarray(XYZ, xp=xp), (2, 3, 1), xp=xp)
        XYZ_p = xp_reshape(xp_asarray(XYZ_p, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(eotf_inverse_DCDM(XYZ), XYZ_p, atol=TOLERANCE_ABSOLUTE_TESTS)
...
dcdm.eotf_inverse_DCDM` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_inverse_DCDM(xp_asarray(XYZ * factor, xp=xp)),
                    XYZ_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
class TestEotf_DCDM:
    ...
    def test_eotf_DCDM(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            eotf_DCDM(xp_asarray(0.0, xp=xp)), 0.0, atol=TOLERANCE_ABSOLUTE_TESTS
        )

        xp_assert_close(
            eotf_DCDM(xp_asarray(0.11281861, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_DCDM(xp_asarray(0.21817973, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_DCDM(xp_asarray(462, xp=xp), in_int=True),
            0.18,
    ...
    def test_n_dimensional_eotf_DCDM(self, xp: ModuleType) -> None:
        ...
        XYZ_p = 0.11281861
        XYZ = np.asarray(eotf_DCDM(xp_asarray(XYZ_p, xp=xp)))
    ...
    def test_domain_range_scale_eotf_DCDM(self, xp: ModuleType) -> None:
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_DCDM(xp_asarray(XYZ_p * factor, xp=xp)),
                    XYZ * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
