...
log_encoding_FLog` definition.
        ...
        xp_assert_close(
            log_encoding_FLog(xp_asarray(0.0, xp=xp)),
            0.092864000000000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog(xp_asarray(0.18, xp=xp)),
            0.459318458661621,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog(xp_asarray(0.18, xp=xp), 12),
            0.459318458661621,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog(xp_asarray(0.18, xp=xp), 10, False),
            0.463336510514656,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog(xp_asarray(0.18, xp=xp), 10, False, False),
            0.446590337236003,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog(xp_asarray(1.0, xp=xp)),
            0.704996409216428,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_FLog` definition n-dimensional arrays support.
        ...
        L_in = 0.18
        V_out = np.asarray(log_encoding_FLog(xp_asarray(L_in, xp=xp)))

        L_in = xp.tile(xp_asarray(L_in, xp=xp), (6,))
        V_out = xp.tile(xp_asarray(V_out, xp=xp), (6,))
        xp_assert_close(log_encoding_FLog(L_in), V_out, atol=TOLERANCE_ABSOLUTE_TESTS)

        L_in = xp_reshape(xp_asarray(L_in, xp=xp), (2, 3), xp=xp)
        V_out = xp_reshape(xp_asarray(V_out, xp=xp), (2, 3), xp=xp)
        xp_assert_close(log_encoding_FLog(L_in), V_out, atol=TOLERANCE_ABSOLUTE_TESTS)

        L_in = xp_reshape(xp_asarray(L_in, xp=xp), (2, 3, 1), xp=xp)
        V_out = xp_reshape(xp_asarray(V_out, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(log_encoding_FLog(L_in), V_out, atol=TOLERANCE_ABSOLUTE_TESTS)
...
log_encoding_FLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_FLog(xp_asarray(L_in * factor, xp=xp)),
                    V_out * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_FLog` definition.
        ...
        xp_assert_close(
            log_decoding_FLog(xp_asarray(0.092864000000000, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog(xp_asarray(0.459318458661621, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog(xp_asarray(0.459318458661621, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog(xp_asarray(0.463336510514656, xp=xp), 10, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog(xp_asarray(0.446590337236003, xp=xp), 10, False, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog(xp_asarray(0.704996409216428, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_FLog` definition n-dimensional arrays support.
        ...
        L_in = np.asarray(log_decoding_FLog(xp_asarray(V_out, xp=xp)))
...
log_decoding_FLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_FLog(xp_asarray(V_out * factor, xp=xp)),
                    L_in * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_encoding_FLog2` definition.
        ...
        xp_assert_close(
            log_encoding_FLog2(xp_asarray(0.0, xp=xp)),
            0.092864000000000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog2(xp_asarray(0.18, xp=xp)),
            0.39100724189123,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog2(xp_asarray(0.18, xp=xp), 12),
            0.39100724189123,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog2(xp_asarray(0.18, xp=xp), 10, False),
            0.383562110108137,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog2(xp_asarray(0.18, xp=xp), 10, False, False),
            0.371293971820387,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_FLog2(xp_asarray(1.0, xp=xp)),
            0.568219370444443,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_FLog2` definition n-dimensional arrays support.
        ...
        L_in = 0.18
        V_out = np.asarray(log_encoding_FLog2(xp_asarray(L_in, xp=xp)))
...
log_encoding_FLog2` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_equal(
                    log_encoding_FLog2(xp_asarray(L_in * factor, xp=xp)),
                    V_out * factor,
                )
...
log_decoding_FLog2` definition.
        ...
        xp_assert_close(
            log_decoding_FLog2(xp_asarray(0.092864000000000, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog2(xp_asarray(0.391007241891230, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog2(xp_asarray(0.391007241891230, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog2(xp_asarray(0.383562110108137, xp=xp), 10, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog2(xp_asarray(0.371293971820387, xp=xp), 10, False, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_FLog2(xp_asarray(0.568219370444443, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_FLog2` definition n-dimensional arrays support.
        ...
        L_in = np.asarray(log_decoding_FLog2(xp_asarray(V_out, xp=xp)))
...
log_decoding_FLog2` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_equal(
                    log_decoding_FLog2(xp_asarray(V_out * factor, xp=xp)),
                    L_in * factor,
                )
