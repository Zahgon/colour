...
log_encoding_MiLog` definition.
        ...
        xp_assert_close(
            log_encoding_MiLog(xp_asarray(0.0, xp=xp)),
            0.14742742,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_MiLog(xp_asarray(0.18, xp=xp)),
            0.45345968,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_MiLog(xp_asarray(0.90, xp=xp)),
            0.66086763,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_MiLog` definition n-dimensional arrays support.
        ...
        R = 0.18
        P = np.asarray(log_encoding_MiLog(xp_asarray(R, xp=xp)))
...
log_encoding_MiLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_MiLog(xp_asarray(R * factor, xp=xp)),
                    P * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_MiLog` definition.
        ...
        xp_assert_close(
            log_decoding_MiLog(xp_asarray(0.14742742, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_MiLog(xp_asarray(0.45345968, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_MiLog(xp_asarray(0.66086763, xp=xp)),
            0.90,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_MiLog` definition n-dimensional arrays support.
        ...
        P = 0.45345968
        R = np.asarray(log_decoding_MiLog(xp_asarray(P, xp=xp)))
...
log_decoding_MiLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_MiLog(xp_asarray(P * factor, xp=xp)),
                    R * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
