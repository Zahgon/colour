...
oetf_BT2100_PQ` definition.
        ...
        xp_assert_close(
            oetf_BT2100_PQ(xp_asarray(0.0, xp=xp)),
            0.000000730955903,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_BT2100_PQ(xp_asarray(0.1, xp=xp)),
            0.724769816665726,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_BT2100_PQ(xp_asarray(1.0, xp=xp)),
            0.999999934308041,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_BT2100_PQ` definition n-dimensional arrays support.
        ...
        E = 0.1
        E_p = np.asarray(oetf_BT2100_PQ(xp_asarray(E, xp=xp)))

        E = xp.tile(xp_asarray(E, xp=xp), (6,))
        E_p = xp.tile(xp_asarray(E_p, xp=xp), (6,))
        xp_assert_close(oetf_BT2100_PQ(E), E_p, atol=TOLERANCE_ABSOLUTE_TESTS)

        E = xp_reshape(xp_asarray(E, xp=xp), (2, 3), xp=xp)
        E_p = xp_reshape(xp_asarray(E_p, xp=xp), (2, 3), xp=xp)
        xp_assert_close(oetf_BT2100_PQ(E), E_p, atol=TOLERANCE_ABSOLUTE_TESTS)

        E = xp_reshape(xp_asarray(E, xp=xp), (2, 3, 1), xp=xp)
        E_p = xp_reshape(xp_asarray(E_p, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(oetf_BT2100_PQ(E), E_p, atol=TOLERANCE_ABSOLUTE_TESTS)
...
oetf_BT2100_PQ` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_BT2100_PQ(xp_asarray(E * factor, xp=xp)),
                    E_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
oetf_inverse_BT2100_PQ` definition.
        ...
        xp_assert_close(
            oetf_inverse_BT2100_PQ(xp_asarray(0.000000730955903, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT2100_PQ(xp_asarray(0.724769816665726, xp=xp)),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT2100_PQ(xp_asarray(0.999999934308041, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_inverse_BT2100_PQ` definition n-dimensional arrays support.
        ...
        E = np.asarray(oetf_inverse_BT2100_PQ(xp_asarray(E_p, xp=xp)))
...
oetf_inverse_BT2100_PQ` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_inverse_BT2100_PQ(xp_asarray(E_p * factor, xp=xp)),
                    E * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_BT2100_PQ` definition.
        ...
        xp_assert_close(
            eotf_BT2100_PQ(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_PQ(xp_asarray(0.724769816665726, xp=xp)),
            779.98836083408537,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_PQ(xp_asarray(1.0, xp=xp)),
            10000.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_BT2100_PQ` definition n-dimensional arrays support.
        ...
        E_p = 0.724769816665726
        F_D = np.asarray(eotf_BT2100_PQ(xp_asarray(E_p, xp=xp)))

        E_p = xp.tile(xp_asarray(E_p, xp=xp), (6,))
        F_D = xp.tile(xp_asarray(F_D, xp=xp), (6,))
        xp_assert_close(eotf_BT2100_PQ(E_p), F_D, atol=TOLERANCE_ABSOLUTE_TESTS)

        E_p = xp_reshape(xp_asarray(E_p, xp=xp), (2, 3), xp=xp)
        F_D = xp_reshape(xp_asarray(F_D, xp=xp), (2, 3), xp=xp)
        xp_assert_close(eotf_BT2100_PQ(E_p), F_D, atol=TOLERANCE_ABSOLUTE_TESTS)

        E_p = xp_reshape(xp_asarray(E_p, xp=xp), (2, 3, 1), xp=xp)
        F_D = xp_reshape(xp_asarray(F_D, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(eotf_BT2100_PQ(E_p), F_D, atol=TOLERANCE_ABSOLUTE_TESTS)
...
eotf_BT2100_PQ` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_BT2100_PQ(xp_asarray(E_p * factor, xp=xp)),
                    F_D * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_inverse_BT2100_PQ` definition.
        ...
        xp_assert_close(
            eotf_inverse_BT2100_PQ(xp_asarray(0.0, xp=xp)),
            0.000000730955903,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_BT2100_PQ(xp_asarray(779.98836083408537, xp=xp)),
            0.724769816665726,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_BT2100_PQ(xp_asarray(10000.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_inverse_BT2100_PQ` definition n-dimensional arrays support.
        ...
        E_p = np.asarray(eotf_inverse_BT2100_PQ(xp_asarray(F_D, xp=xp)))
...
eotf_inverse_BT2100_PQ` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_inverse_BT2100_PQ(xp_asarray(F_D * factor, xp=xp)),
                    E_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
ootf_BT2100_PQ` definition.
        ...
        xp_assert_close(
            ootf_BT2100_PQ(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_PQ(xp_asarray(0.1, xp=xp)),
            779.98836083411584,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_PQ(xp_asarray(1.0, xp=xp)),
...
ootf_BT2100_PQ` definition n-dimensional arrays support.
        ...
        E = 0.1
        F_D = np.asarray(ootf_BT2100_PQ(xp_asarray(E, xp=xp)))
...
ootf_BT2100_PQ` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    ootf_BT2100_PQ(xp_asarray(E * factor, xp=xp)),
                    F_D * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
ootf_inverse_BT2100_PQ` definition.
        ...
        xp_assert_close(
            ootf_inverse_BT2100_PQ(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_inverse_BT2100_PQ(xp_asarray(779.98836083411584, xp=xp)),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_inverse_BT2100_PQ(xp_asarray(9999.993723673924300, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
ootf_inverse_BT2100_PQ` definition n-dimensional arrays support.
        ...
        E = np.asarray(ootf_inverse_BT2100_PQ(xp_asarray(F_D, xp=xp)))
...
ootf_inverse_BT2100_PQ` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    ootf_inverse_BT2100_PQ(xp_asarray(F_D * factor, xp=xp)),
                    E * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
oetf_BT2100_HLG` definition.
        ...
        xp_assert_close(
            oetf_BT2100_HLG(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_BT2100_HLG(xp_asarray(0.18 / 12, xp=xp)),
            0.212132034355964,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_BT2100_HLG(xp_asarray(1.0, xp=xp)),
            0.999999995536569,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_BT2100_HLG` definition n-dimensional arrays support.
        ...
        E = 0.18 / 12
        E_p = np.asarray(oetf_BT2100_HLG(xp_asarray(E, xp=xp)))
...
oetf_BT2100_HLG` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_BT2100_HLG(xp_asarray(E * factor, xp=xp)),
                    E_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
oetf_inverse_BT2100_HLG` definition.
        ...
        xp_assert_close(
            oetf_inverse_BT2100_HLG(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT2100_HLG(xp_asarray(0.212132034355964, xp=xp)),
            0.18 / 12,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT2100_HLG(xp_asarray(0.999999995536569, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_inverse_BT2100_HLG` definition n-dimensional arrays support.
        ...
        E = np.asarray(oetf_inverse_BT2100_HLG(xp_asarray(E_p, xp=xp)))
...
oetf_inverse_BT2100_HLG` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_inverse_BT2100_HLG(xp_asarray(E_p * factor, xp=xp)),
                    E * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_BT2100_HLG_1` definition.
        ...
        xp_assert_close(
            eotf_BT2100_HLG_1(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_HLG_1(xp_asarray(0.212132034355964, xp=xp)),
            6.476039825649814,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_HLG_1(xp_asarray(1.0, xp=xp)),
            1000.000032321769100,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_HLG_1(xp_asarray(0.212132034355964, xp=xp), 0.001, 10000, 1.4),
            27.96039175299561,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_BT2100_HLG_1` definition n-dimensional arrays support.
        ...
        E_p = 0.212132034355964
        F_D = np.asarray(eotf_BT2100_HLG_1(xp_asarray(E_p, xp=xp)))
        ...
        E_p = xp_reshape(xp_asarray(E_p, xp=xp), (6, 1), xp=xp)
        F_D = xp_reshape(xp_asarray(F_D, xp=xp), (6, 1), xp=xp)
        xp_assert_close(eotf_BT2100_HLG_1(E_p), F_D, atol=TOLERANCE_ABSOLUTE_TESTS)
        ...
        E_p = xp.tile(xp_asarray(E_p, xp=xp), (6, 1))
        F_D = xp.tile(xp_asarray(F_D, xp=xp), (6, 1))
        xp_assert_close(eotf_BT2100_HLG_1(E_p), F_D, atol=TOLERANCE_ABSOLUTE_TESTS)

        E_p = xp_reshape(xp_asarray(E_p, xp=xp), (2, 3, 3), xp=xp)
        F_D = xp_reshape(xp_asarray(F_D, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(eotf_BT2100_HLG_1(E_p), F_D, atol=TOLERANCE_ABSOLUTE_TESTS)
...
eotf_BT2100_HLG_1` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_BT2100_HLG_1(xp_asarray(E_p * factor, xp=xp)),
                    F_D * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_BT2100_HLG_2` definition.
        ...
        xp_assert_close(
            eotf_BT2100_HLG_2(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_HLG_2(xp_asarray(0.212132034355964, xp=xp)),
            6.476039825649814,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_HLG_2(xp_asarray(1.0, xp=xp)),
            1000.000032321769100,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_BT2100_HLG_2(xp_asarray(0.212132034355964, xp=xp), 0.001, 10000, 1.4),
            29.581261576946076,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_BT2100_HLG_2` definition n-dimensional arrays support.
        ...
        E_p = 0.212132034355964
        F_D = np.asarray(eotf_BT2100_HLG_2(xp_asarray(E_p, xp=xp)))
...
eotf_BT2100_HLG_2` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_BT2100_HLG_2(xp_asarray(E_p * factor, xp=xp)),
                    F_D * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_inverse_BT2100_HLG_1` definition.
        ...
        xp_assert_close(
            eotf_inverse_BT2100_HLG_1(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_BT2100_HLG_1(xp_asarray(6.476039825649814, xp=xp)),
            0.212132034355964,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_BT2100_HLG_1(xp_asarray(1000.000032321769100, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            eotf_inverse_BT2100_HLG_1(
                xp_asarray(27.96039175299561, xp=xp), 0.001, 10000, 1.4
            ),
            0.212132034355964,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_inverse_BT2100_HLG_1` definition n-dimensional arrays support.
        ...
        E_p = np.asarray(eotf_inverse_BT2100_HLG_1(xp_asarray(F_D, xp=xp)))
...
eotf_inverse_BT2100_HLG_1` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_inverse_BT2100_HLG_1(xp_asarray(F_D * factor, xp=xp)),
                    E_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_inverse_BT2100_HLG_2` definition.
        ...
        xp_assert_close(
            eotf_inverse_BT2100_HLG_2(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_BT2100_HLG_2(xp_asarray(6.476039825649814, xp=xp)),
            0.212132034355964,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_BT2100_HLG_2(xp_asarray(1000.000032321769100, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            eotf_inverse_BT2100_HLG_2(
                xp_asarray(29.581261576946076, xp=xp), 0.001, 10000, 1.4
            ),
            0.212132034355964,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_inverse_BT2100_HLG_2` definition n-dimensional arrays support.
        ...
        F_D = 6.476039825649814
        E_p = np.asarray(eotf_inverse_BT2100_HLG_2(xp_asarray(F_D, xp=xp)))
...
eotf_inverse_BT2100_HLG_2` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_inverse_BT2100_HLG_2(xp_asarray(F_D * factor, xp=xp)),
                    E_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
ootf_BT2100_HLG_1` definition.
        ...
        xp_assert_close(
            ootf_BT2100_HLG_1(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_HLG_1(xp_asarray(0.1, xp=xp)),
            63.095734448019336,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_HLG_1(xp_asarray(1.0, xp=xp)),
            1000.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_HLG_1(xp_asarray(0.1, xp=xp), 0.001, 10000, 1.4),
            398.108130742780300,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
ootf_BT2100_HLG_1` definition n-dimensional arrays support.
        ...
        E = 0.1
        F_D = np.asarray(ootf_BT2100_HLG_1(xp_asarray(E, xp=xp)))
        ...
        E = xp_reshape(xp_asarray(E, xp=xp), (6, 1), xp=xp)
        F_D = xp_reshape(xp_asarray(F_D, xp=xp), (6, 1), xp=xp)
        xp_assert_close(ootf_BT2100_HLG_1(E), F_D, atol=TOLERANCE_ABSOLUTE_TESTS)
...
ootf_BT2100_HLG_1` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    ootf_BT2100_HLG_1(xp_asarray(E * factor, xp=xp)),
                    F_D * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
ootf_BT2100_HLG_2` definition.
        ...
        xp_assert_close(
            ootf_BT2100_HLG_2(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_HLG_2(xp_asarray(0.1, xp=xp)),
            63.095734448019336,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_HLG_2(xp_asarray(1.0, xp=xp)),
            1000.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_BT2100_HLG_2(xp_asarray(0.1, xp=xp), 10000, 1.4),
            398.107170553497380,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
ootf_BT2100_HLG_2` definition n-dimensional arrays support.
        ...
        E = 0.1
        F_D = np.asarray(ootf_BT2100_HLG_2(xp_asarray(E, xp=xp)))
...
ootf_inverse_BT2100_HLG_1` definition.
        ...
        xp_assert_close(
            ootf_inverse_BT2100_HLG_1(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_inverse_BT2100_HLG_1(xp_asarray(63.095734448019336, xp=xp)),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_inverse_BT2100_HLG_1(xp_asarray(1000.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ootf_inverse_BT2100_HLG_1(
                xp_asarray(398.108130742780300, xp=xp), 0.001, 10000, 1.4
            ),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ootf_inverse_BT2100_HLG_1(xp_asarray(a, xp=xp)),
            np.array(
                [
                    [0.1, 0.0, -0.1],
                    [-0.1, -0.1, -0.1],
...
ootf_inverse_BT2100_HLG_1` definition n-dimensional arrays support.
        ...
        E = np.asarray(ootf_inverse_BT2100_HLG_1(xp_asarray(F_D, xp=xp)))
...
ootf_inverse_BT2100_HLG_1` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    ootf_inverse_BT2100_HLG_1(xp_asarray(F_D * factor, xp=xp)),
                    E * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
ootf_inverse_BT2100_HLG_2` definition.
        ...
        xp_assert_close(
            ootf_inverse_BT2100_HLG_2(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_inverse_BT2100_HLG_2(xp_asarray(63.095734448019336, xp=xp)),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ootf_inverse_BT2100_HLG_2(xp_asarray(1000.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ootf_inverse_BT2100_HLG_2(
                xp_asarray(398.107170553497380, xp=xp), 10000, 1.4
            ),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ootf_inverse_BT2100_HLG_2(xp_asarray(a, xp=xp)),
            np.array(
                [
                    [0.1, 0.0, -0.1],
                    [-0.1, -0.1, -0.1],
...
ootf_inverse_BT2100_HLG_2` definition n-dimensional arrays support.
        ...
        F_D = 63.095734448019336
        E = np.asarray(ootf_inverse_BT2100_HLG_2(xp_asarray(F_D, xp=xp)))
...
ootf_inverse_BT2100_HLG_2` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    ootf_inverse_BT2100_HLG_2(xp_asarray(F_D * factor, xp=xp)),
                    E * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
ootf_BT2100_HLG` definition.
        ...
        xp_assert_close(
            ootf_BT2100_HLG(xp_asarray(0.1, xp=xp)),
            63.095734448019336,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ootf_BT2100_HLG(xp_asarray(0.1, xp=xp), 0.01, method="ITU-R BT.2100-1"),
            63.105103490674857,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ootf_BT2100_HLG(xp_asarray(0.1, xp=xp), L_W=2000),
            94.3186112317,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
ootf_inverse_BT2100_HLG` definition.
        ...
        # Test default method (ITU-R BT.2100-2)
        xp_assert_close(
            ootf_inverse_BT2100_HLG(xp_asarray(63.095734448019336, xp=xp)),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ootf_inverse_BT2100_HLG(
                xp_asarray(63.105103490674857, xp=xp), 0.01, method="ITU-R BT.2100-1"
            ),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        # Test with different L_W value
        xp_assert_close(
            ootf_inverse_BT2100_HLG(xp_asarray(94.3186112317, xp=xp), L_W=2000),
            0.1,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
