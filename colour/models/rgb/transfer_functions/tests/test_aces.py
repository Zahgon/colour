...
log_encoding_ACESproxy` definition.
        ...
        xp_assert_close(
            log_encoding_ACESproxy(xp_asarray(0.0, xp=xp)),
            0.062561094819159,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ACESproxy(xp_asarray(0.18, xp=xp)),
            0.416422287390029,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ACESproxy(xp_asarray(0.18, xp=xp), 12),
            0.416361416361416,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ACESproxy(xp_asarray(1.0, xp=xp)),
            0.537634408602151,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        assert (
            np.asarray(log_encoding_ACESproxy(xp_asarray(0.18, xp=xp), out_int=True))
            == 426
        )
...
log_encoding_ACESproxy` definition n-dimensional arrays support.
        ...
        lin_AP1 = 0.18
        ACESproxy = np.asarray(log_encoding_ACESproxy(xp_asarray(lin_AP1, xp=xp)))

        lin_AP1 = xp.tile(xp_asarray(lin_AP1, xp=xp), (6,))
        ACESproxy = xp.tile(xp_asarray(ACESproxy, xp=xp), (6,))
        xp_assert_close(
        ...
        lin_AP1 = xp_reshape(xp_asarray(lin_AP1, xp=xp), (2, 3), xp=xp)
        ACESproxy = xp_reshape(xp_asarray(ACESproxy, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            log_encoding_ACESproxy(lin_AP1),
            ACESproxy,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        lin_AP1 = xp_reshape(xp_asarray(lin_AP1, xp=xp), (2, 3, 1), xp=xp)
        ACESproxy = xp_reshape(xp_asarray(ACESproxy, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(
            log_encoding_ACESproxy(lin_AP1),
            ACESproxy,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
log_encoding_ACESproxy` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_ACESproxy(xp_asarray(lin_AP1 * factor, xp=xp)),
                    ACESproxy * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_ACESproxy` definition.
        ...
        xp_assert_close(
            log_decoding_ACESproxy(xp_asarray(0.062561094819159, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS * 100000,
        )

        xp_assert_close(
            log_decoding_ACESproxy(xp_asarray(0.416422287390029, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 100000,
        )

        xp_assert_close(
            log_decoding_ACESproxy(xp_asarray(0.416361416361416, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 100000,
        )

        xp_assert_close(
            log_decoding_ACESproxy(xp_asarray(0.537634408602151, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS * 100000,
        )

        xp_assert_close(
            log_decoding_ACESproxy(xp_asarray(426, xp=xp), in_int=True),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 100000,
        )
...
log_decoding_ACESproxy` definition n-dimensional arrays support.
        ...
        lin_AP1 = np.asarray(log_decoding_ACESproxy(xp_asarray(ACESproxy, xp=xp)))
...
log_decoding_ACESproxy` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_ACESproxy(xp_asarray(ACESproxy * factor, xp=xp)),
                    lin_AP1 * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_encoding_ACEScc` definition.
        ...
        xp_assert_close(
            log_encoding_ACEScc(xp_asarray(0.0, xp=xp)),
            -0.358447488584475,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ACEScc(xp_asarray(0.18, xp=xp)),
            0.413588402492442,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ACEScc(xp_asarray(1.0, xp=xp)),
            0.554794520547945,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_ACEScc` definition n-dimensional arrays support.
        ...
        lin_AP1 = 0.18
        ACEScc = np.asarray(log_encoding_ACEScc(xp_asarray(lin_AP1, xp=xp)))

        lin_AP1 = xp.tile(xp_asarray(lin_AP1, xp=xp), (6,))
        ACEScc = xp.tile(xp_asarray(ACEScc, xp=xp), (6,))
        xp_assert_close(
        ...
        lin_AP1 = xp_reshape(xp_asarray(lin_AP1, xp=xp), (2, 3), xp=xp)
        ACEScc = xp_reshape(xp_asarray(ACEScc, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            log_encoding_ACEScc(lin_AP1),
            ACEScc,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        lin_AP1 = xp_reshape(xp_asarray(lin_AP1, xp=xp), (2, 3, 1), xp=xp)
        ACEScc = xp_reshape(xp_asarray(ACEScc, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(
            log_encoding_ACEScc(lin_AP1),
            ACEScc,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
log_encoding_ACEScc` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_ACEScc(xp_asarray(lin_AP1 * factor, xp=xp)),
                    ACEScc * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_ACEScc` definition.
        ...
        xp_assert_close(
            log_decoding_ACEScc(xp_asarray(-0.358447488584475, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_ACEScc(xp_asarray(0.413588402492442, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_ACEScc(xp_asarray(0.554794520547945, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_ACEScc` definition n-dimensional arrays support.
        ...
        lin_AP1 = np.asarray(log_decoding_ACEScc(xp_asarray(ACEScc, xp=xp)))
...
log_decoding_ACEScc` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_ACEScc(xp_asarray(ACEScc * factor, xp=xp)),
                    lin_AP1 * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_encoding_ACEScct` definition.
        ...
        xp_assert_close(
            log_encoding_ACEScct(xp_asarray(0.0, xp=xp)),
            0.072905534195835495,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ACEScct(xp_asarray(0.18, xp=xp)),
            0.413588402492442,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ACEScct(xp_asarray(1.0, xp=xp)),
            0.554794520547945,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_ACEScct` definition n-dimensional arrays support.
        ...
        lin_AP1 = 0.18
        ACEScct = np.asarray(log_encoding_ACEScct(xp_asarray(lin_AP1, xp=xp)))

        lin_AP1 = xp.tile(xp_asarray(lin_AP1, xp=xp), (6,))
        ACEScct = xp.tile(xp_asarray(ACEScct, xp=xp), (6,))
        xp_assert_close(
        ...
        lin_AP1 = xp_reshape(xp_asarray(lin_AP1, xp=xp), (2, 3), xp=xp)
        ACEScct = xp_reshape(xp_asarray(ACEScct, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            log_encoding_ACEScct(lin_AP1),
            ACEScct,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        lin_AP1 = xp_reshape(xp_asarray(lin_AP1, xp=xp), (2, 3, 1), xp=xp)
        ACEScct = xp_reshape(xp_asarray(ACEScct, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(
            log_encoding_ACEScct(lin_AP1),
            ACEScct,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
log_encoding_ACEScct` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_ACEScct(xp_asarray(lin_AP1 * factor, xp=xp)),
                    ACEScct * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
S-2016-001/introduction.tex#L14)
        ...
        xp_assert_close(
            log_encoding_ACEScct(xp_asarray(equiv, xp=xp)),
            np.asarray(log_encoding_ACEScc(xp_asarray(equiv, xp=xp))),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_ACEScct` definition.
        ...
        xp_assert_close(
            log_decoding_ACEScct(xp_asarray(0.072905534195835495, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_ACEScct(xp_asarray(0.41358840249244228, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_ACEScct(xp_asarray(0.554794520547945, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_ACEScct` definition n-dimensional arrays support.
        ...
        lin_AP1 = np.asarray(log_decoding_ACEScct(xp_asarray(ACEScct, xp=xp)))
...
log_decoding_ACEScct` definition domain and range scale support.
        ...
        ACEScc = 0.413588402492442
        lin_AP1 = np.asarray(log_decoding_ACEScct(xp_asarray(ACEScc, xp=xp)))

        d_r = (("reference", 1), ("1", 1), ("100", 100))
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_ACEScct(xp_asarray(ACEScc * factor, xp=xp)),
                    lin_AP1 * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
S-2016-001/introduction.tex#L14)
        ...
        xp_assert_close(
            log_decoding_ACEScct(xp_asarray(equiv, xp=xp)),
            np.asarray(log_decoding_ACEScc(xp_asarray(equiv, xp=xp))),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
