...
cctf_encoding_ROMMRGB` definition.
        ...
        xp_assert_close(
            cctf_encoding_ROMMRGB(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_encoding_ROMMRGB(xp_asarray(0.18, xp=xp)),
            0.385711424751138,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_encoding_ROMMRGB(xp_asarray(1.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        assert (
            np.asarray(cctf_encoding_ROMMRGB(xp_asarray(0.18, xp=xp), out_int=True))
            == 98
        )
        ...
        assert (
            np.asarray(
                cctf_encoding_ROMMRGB(
                    xp_asarray(0.18, xp=xp), bit_depth=12, out_int=True
                )
            )
            == 1579
        )
...
cctf_encoding_ROMMRGB` definition n-dimensional arrays support.
        ...
        X = 0.18
        X_ROMM = np.asarray(cctf_encoding_ROMMRGB(xp_asarray(X, xp=xp)))

        X = xp.tile(xp_asarray(X, xp=xp), (6,))
        X_ROMM = xp.tile(xp_asarray(X_ROMM, xp=xp), (6,))
        xp_assert_close(cctf_encoding_ROMMRGB(X), X_ROMM, atol=TOLERANCE_ABSOLUTE_TESTS)

        X = xp_reshape(xp_asarray(X, xp=xp), (2, 3), xp=xp)
        X_ROMM = xp_reshape(xp_asarray(X_ROMM, xp=xp), (2, 3), xp=xp)
        xp_assert_close(cctf_encoding_ROMMRGB(X), X_ROMM, atol=TOLERANCE_ABSOLUTE_TESTS)

        X = xp_reshape(xp_asarray(X, xp=xp), (2, 3, 1), xp=xp)
        X_ROMM = xp_reshape(xp_asarray(X_ROMM, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(cctf_encoding_ROMMRGB(X), X_ROMM, atol=TOLERANCE_ABSOLUTE_TESTS)
...
cctf_encoding_ROMMRGB` definition domain and range scale support.
        ...
        X = 0.18
        X_p = np.asarray(cctf_encoding_ROMMRGB(xp_asarray(X, xp=xp)))

        d_r = (("reference", 1), ("1", 1), ("100", 100))
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    cctf_encoding_ROMMRGB(xp_asarray(X * factor, xp=xp)),
                    X_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
cctf_decoding_ROMMRGB` definition.
        ...
        xp_assert_close(
            cctf_decoding_ROMMRGB(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_decoding_ROMMRGB(xp_asarray(0.385711424751138, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_decoding_ROMMRGB(xp_asarray(1.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_decoding_ROMMRGB(xp_asarray(98, xp=xp), in_int=True),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 100000,
        )

        xp_assert_close(
            cctf_decoding_ROMMRGB(xp_asarray(1579, xp=xp), bit_depth=12, in_int=True),
            0.18,
...
cctf_decoding_ROMMRGB` definition n-dimensional arrays support.
        ...
        X = np.asarray(cctf_decoding_ROMMRGB(xp_asarray(X_p, xp=xp)))

        X_p = xp.tile(xp_asarray(X_p, xp=xp), (6,))
        X = xp.tile(xp_asarray(X, xp=xp), (6,))
        xp_assert_close(cctf_decoding_ROMMRGB(X_p), X, atol=TOLERANCE_ABSOLUTE_TESTS)

        X_p = xp_reshape(xp_asarray(X_p, xp=xp), (2, 3), xp=xp)
        X = xp_reshape(xp_asarray(X, xp=xp), (2, 3), xp=xp)
        xp_assert_close(cctf_decoding_ROMMRGB(X_p), X, atol=TOLERANCE_ABSOLUTE_TESTS)

        X_p = xp_reshape(xp_asarray(X_p, xp=xp), (2, 3, 1), xp=xp)
        X = xp_reshape(xp_asarray(X, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(cctf_decoding_ROMMRGB(X_p), X, atol=TOLERANCE_ABSOLUTE_TESTS)
...
cctf_decoding_ROMMRGB` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    cctf_decoding_ROMMRGB(xp_asarray(X_p * factor, xp=xp)),
                    X * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
cctf_encoding_RIMMRGB` definition.
        ...
        xp_assert_close(
            cctf_encoding_RIMMRGB(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_encoding_RIMMRGB(xp_asarray(0.18, xp=xp)),
            0.291673732475746,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_encoding_RIMMRGB(xp_asarray(1.0, xp=xp)),
            0.713125234297525,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        assert (
            np.asarray(cctf_encoding_RIMMRGB(xp_asarray(0.18, xp=xp), out_int=True))
            == 74
        )
...
cctf_encoding_RIMMRGB` definition n-dimensional arrays support.
        ...
        X = 0.18
        X_p = np.asarray(cctf_encoding_RIMMRGB(xp_asarray(X, xp=xp)))
...
cctf_encoding_RIMMRGB` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    cctf_encoding_RIMMRGB(xp_asarray(X * factor, xp=xp)),
                    X_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
cctf_decoding_RIMMRGB` definition.
        ...
        xp_assert_close(
            cctf_decoding_RIMMRGB(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_decoding_RIMMRGB(xp_asarray(0.291673732475746, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_decoding_RIMMRGB(xp_asarray(0.713125234297525, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            cctf_decoding_RIMMRGB(xp_asarray(74, xp=xp), in_int=True),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 50000,
        )

        xp_assert_close(
            cctf_decoding_RIMMRGB(xp_asarray(1194, xp=xp), bit_depth=12, in_int=True),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 50000,
        )
...
cctf_decoding_RIMMRGB` definition n-dimensional arrays support.
        ...
        X = np.asarray(cctf_decoding_RIMMRGB(xp_asarray(X_p, xp=xp)))
...
cctf_decoding_RIMMRGB` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    cctf_decoding_RIMMRGB(xp_asarray(X_p * factor, xp=xp)),
                    X * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_encoding_ERIMMRGB` definition.
        ...
        xp_assert_close(
            log_encoding_ERIMMRGB(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ERIMMRGB(xp_asarray(0.18, xp=xp)),
            0.410052389492129,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_ERIMMRGB(xp_asarray(1.0, xp=xp)),
            0.545458327405113,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        assert (
            np.asarray(log_encoding_ERIMMRGB(xp_asarray(0.18, xp=xp), out_int=True))
            == 105
        )
...
log_encoding_ERIMMRGB` definition n-dimensional arrays support.
        ...
        X = 0.18
        X_p = np.asarray(log_encoding_ERIMMRGB(xp_asarray(X, xp=xp)))
...
log_encoding_ERIMMRGB` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_ERIMMRGB(xp_asarray(X * factor, xp=xp)),
                    X_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_ERIMMRGB` definition.
        ...
        xp_assert_close(
            log_decoding_ERIMMRGB(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_ERIMMRGB(xp_asarray(0.410052389492129, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_ERIMMRGB(xp_asarray(0.545458327405113, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_ERIMMRGB(xp_asarray(105, xp=xp), in_int=True),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 50000,
        )

        xp_assert_close(
            log_decoding_ERIMMRGB(xp_asarray(1679, xp=xp), bit_depth=12, in_int=True),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS * 50000,
        )
...
log_decoding_ERIMMRGB` definition n-dimensional arrays support.
        ...
        X = np.asarray(log_decoding_ERIMMRGB(xp_asarray(X_p, xp=xp)))
...
log_decoding_ERIMMRGB` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_ERIMMRGB(xp_asarray(X_p * factor, xp=xp)),
                    X * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
