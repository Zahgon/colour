...
oetf_ARIBSTDB67` definition.
        ...
        xp_assert_close(
            oetf_ARIBSTDB67(xp_asarray(-0.25, xp=xp)),
            -0.25,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_ARIBSTDB67(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_ARIBSTDB67(xp_asarray(0.18, xp=xp)),
            0.212132034355964,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_ARIBSTDB67(xp_asarray(1.0, xp=xp)),
            0.5,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_ARIBSTDB67(xp_asarray(64.0, xp=xp)),
            1.302858098046995,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_ARIBSTDB67` definition n-dimensional arrays support.
        ...
        E = 0.18
        E_p = np.asarray(oetf_ARIBSTDB67(xp_asarray(E, xp=xp)))
...
oetf_ARIBSTDB67` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_ARIBSTDB67(xp_asarray(E * factor, xp=xp)),
                    E_p * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
oetf_inverse_ARIBSTDB67` definition.
        ...
        xp_assert_close(
            oetf_inverse_ARIBSTDB67(xp_asarray(-0.25, xp=xp)),
            -0.25,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_ARIBSTDB67(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_ARIBSTDB67(xp_asarray(0.212132034355964, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_ARIBSTDB67(xp_asarray(0.5, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_ARIBSTDB67(xp_asarray(1.302858098046995, xp=xp)),
            64.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_inverse_ARIBSTDB67` definition n-dimensional arrays support.
        ...
        E = np.asarray(oetf_inverse_ARIBSTDB67(xp_asarray(E_p, xp=xp)))
...
oetf_inverse_ARIBSTDB67` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_inverse_ARIBSTDB67(xp_asarray(E_p * factor, xp=xp)),
                    E * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
