...
eotf_inverse_sRGB` definition.
        ...
        xp_assert_close(
            eotf_inverse_sRGB(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_sRGB(xp_asarray(0.18, xp=xp)),
            0.461356129500442,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_inverse_sRGB(xp_asarray(1.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_inverse_sRGB` definition n-dimensional arrays support.
        ...
        L = 0.18
        V = np.asarray(eotf_inverse_sRGB(xp_asarray(L, xp=xp)))
...
eotf_inverse_sRGB` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_inverse_sRGB(xp_asarray(L * factor, xp=xp)),
                    V * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_sRGB` definition.
        ...
        xp_assert_close(
            eotf_sRGB(xp_asarray(0.0, xp=xp)), 0.0, atol=TOLERANCE_ABSOLUTE_TESTS
        )

        xp_assert_close(
            eotf_sRGB(xp_asarray(0.461356129500442, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_sRGB(xp_asarray(1.0, xp=xp)), 1.0, atol=TOLERANCE_ABSOLUTE_TESTS
        )
...
eotf_sRGB` definition n-dimensional arrays support.
        ...
        V = 0.461356129500442
        L = np.asarray(eotf_sRGB(xp_asarray(V, xp=xp)))
...
eotf_sRGB` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_sRGB(xp_asarray(V * factor, xp=xp)),
                    L * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
