...
log_encoding_NLog` definition.
        ...
        xp_assert_close(
            log_encoding_NLog(xp_asarray(0.0, xp=xp)),
            0.124372627896372,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_NLog(xp_asarray(0.18, xp=xp)),
            0.363667770117139,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_NLog(xp_asarray(0.18, xp=xp), 12),
            0.363667770117139,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_NLog(xp_asarray(0.18, xp=xp), 10, False),
            0.351634850262366,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_NLog(xp_asarray(0.18, xp=xp), 10, False, False),
            0.337584957293328,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_NLog(xp_asarray(1.0, xp=xp)),
            0.605083088954056,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_NLog` definition n-dimensional arrays support.
        ...
        y = 0.18
        x = np.asarray(log_encoding_NLog(xp_asarray(y, xp=xp)))
...
log_encoding_NLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_NLog(xp_asarray(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_NLog` definition.
        ...
        xp_assert_close(
            log_decoding_NLog(xp_asarray(0.124372627896372, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_NLog(xp_asarray(0.363667770117139, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_NLog(xp_asarray(0.363667770117139, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_NLog(xp_asarray(0.351634850262366, xp=xp), 10, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_NLog(xp_asarray(0.337584957293328, xp=xp), 10, False, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_NLog(xp_asarray(0.605083088954056, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_NLog` definition n-dimensional arrays support.
        ...
        x = 0.363667770117139
        y = np.asarray(log_decoding_NLog(xp_asarray(x, xp=xp)))
...
log_decoding_NLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_NLog(xp_asarray(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
