...
oetf_BT709` definition.
        ...
        xp_assert_close(
            oetf_BT709(xp_asarray(0.0, xp=xp)), 0.0, atol=TOLERANCE_ABSOLUTE_TESTS
        )

        xp_assert_close(
            oetf_BT709(xp_asarray(0.015, xp=xp)),
            0.067500000000000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_BT709(xp_asarray(0.18, xp=xp)),
            0.409007728864150,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_BT709(xp_asarray(1.0, xp=xp)), 1.0, atol=TOLERANCE_ABSOLUTE_TESTS
        )
...
oetf_BT709` definition n-dimensional arrays support.
        ...
        L = 0.18
        V = np.asarray(oetf_BT709(xp_asarray(L, xp=xp)))
...
oetf_BT709` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_BT709(xp_asarray(L * factor, xp=xp)),
                    V * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
oetf_inverse_BT709` definition.
        ...
        xp_assert_close(
            oetf_inverse_BT709(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT709(xp_asarray(0.067500000000000, xp=xp)),
            0.015,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT709(xp_asarray(0.409007728864150, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_BT709(xp_asarray(1.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_inverse_BT709` definition n-dimensional arrays support.
        ...
        V = 0.409007728864150
        L = np.asarray(oetf_inverse_BT709(xp_asarray(V, xp=xp)))
...
oetf_inverse_BT709` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_inverse_BT709(xp_asarray(V * factor, xp=xp)),
                    L * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
