...
davinci_intermediate.oetf_DaVinciIntermediate` definition.
        ...
        xp_assert_close(
            oetf_DaVinciIntermediate(xp_asarray(-0.01, xp=xp)),
            -0.104442685500000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_DaVinciIntermediate(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_DaVinciIntermediate(xp_asarray(0.18, xp=xp)),
            0.336043272384855,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_DaVinciIntermediate(xp_asarray(1.0, xp=xp)),
            0.513837441116225,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_DaVinciIntermediate(xp_asarray(100.0, xp=xp)),
            0.999999987016872,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
davinci_intermediate.oetf_DaVinciIntermediate` definition n-dimensional arrays
        ...
        L = 0.18
        V = np.asarray(oetf_DaVinciIntermediate(xp_asarray(L, xp=xp)))
...
davinci_intermediate.oetf_DaVinciIntermediate` definition domain and range
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_DaVinciIntermediate(xp_asarray(L * factor, xp=xp)),
                    V * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
davinci_intermediate.oetf_inverse_DaVinciIntermediate` definition.
        ...
        xp_assert_close(
            oetf_inverse_DaVinciIntermediate(xp_asarray(-0.104442685500000, xp=xp)),
            -0.01,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_DaVinciIntermediate(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_DaVinciIntermediate(xp_asarray(0.336043272384855, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_DaVinciIntermediate(xp_asarray(0.513837441116225, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_inverse_DaVinciIntermediate(xp_asarray(0.999999987016872, xp=xp)),
            100.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
davinci_intermediate.oetf_inverse_DaVinciIntermediate` definition n-dimensional
        ...
        V = 0.336043272384855
        L = np.asarray(oetf_inverse_DaVinciIntermediate(xp_asarray(V, xp=xp)))
...
davinci_intermediate.oetf_inverse_DaVinciIntermediate` definition domain and
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_inverse_DaVinciIntermediate(xp_asarray(V * factor, xp=xp)),
                    L * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
