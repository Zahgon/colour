...
log_encoding_LLog` definition.
        ...
        xp_assert_close(
            log_encoding_LLog(xp_asarray(0.0, xp=xp)),
            0.089999999999999,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_LLog(xp_asarray(0.18, xp=xp)),
            0.435313904043927,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_LLog(xp_asarray(0.18, xp=xp), 12),
            0.435313904043927,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_LLog(xp_asarray(0.18, xp=xp), 10, False),
            0.4353037943344028,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_LLog(xp_asarray(0.18, xp=xp), 10, False, False),
            0.421586960452824,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_LLog(xp_asarray(1.0, xp=xp)),
            0.631797439630121,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_LLog` definition n-dimensional arrays support.
        ...
        y = 0.18
        x = np.asarray(log_encoding_LLog(xp_asarray(y, xp=xp)))
...
log_encoding_LLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_LLog(xp_asarray(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_LLog` definition.
        ...
        xp_assert_close(
            log_decoding_LLog(xp_asarray(0.089999999999999, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_LLog(xp_asarray(0.435313904043927, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_LLog(xp_asarray(0.435313904043927, xp=xp), 12),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_LLog(xp_asarray(0.4353037943344028, xp=xp), 10, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_LLog(xp_asarray(0.421586960452824, xp=xp), 10, False, False),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_LLog(xp_asarray(0.631797439630121, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_LLog` definition n-dimensional arrays support.
        ...
        x = 0.435313904043927
        y = np.asarray(log_decoding_LLog(xp_asarray(x, xp=xp)))
...
log_decoding_LLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_LLog(xp_asarray(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
