...
log_encoding_Panalog` definition.
        ...
        xp_assert_close(
            log_encoding_Panalog(xp_asarray(0.0, xp=xp)),
            0.062561094819159,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_Panalog(xp_asarray(0.18, xp=xp)),
            0.374576791382298,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_Panalog(xp_asarray(1.0, xp=xp)),
            0.665689149560117,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_Panalog` definition n-dimensional arrays support.
        ...
        x = 0.18
        y = np.asarray(log_encoding_Panalog(xp_asarray(x, xp=xp)))
...
log_encoding_Panalog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_Panalog(xp_asarray(x * factor, xp=xp)),
                    y * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_Panalog` definition.
        ...
        xp_assert_close(
            log_decoding_Panalog(xp_asarray(0.062561094819159, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_Panalog(xp_asarray(0.374576791382298, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_Panalog(xp_asarray(0.665689149560117, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_Panalog` definition n-dimensional arrays support.
        ...
        y = 0.374576791382298
        x = np.asarray(log_decoding_Panalog(xp_asarray(y, xp=xp)))
...
log_decoding_Panalog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_Panalog(xp_asarray(y * factor, xp=xp)),
                    x * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
