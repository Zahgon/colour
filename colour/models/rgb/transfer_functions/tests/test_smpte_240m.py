...
oetf_SMPTE240M` definition.
        ...
        xp_assert_close(
            oetf_SMPTE240M(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_SMPTE240M(xp_asarray(0.02, xp=xp)),
            0.080000000000000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_SMPTE240M(xp_asarray(0.18, xp=xp)),
            0.402285796753870,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            oetf_SMPTE240M(xp_asarray(1.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
oetf_SMPTE240M` definition n-dimensional arrays support.
        ...
        L_c = 0.18
        V_c = np.asarray(oetf_SMPTE240M(xp_asarray(L_c, xp=xp)))

        L_c = xp.tile(xp_asarray(L_c, xp=xp), (6,))
        V_c = xp.tile(xp_asarray(V_c, xp=xp), (6,))
        xp_assert_close(oetf_SMPTE240M(L_c), V_c, atol=TOLERANCE_ABSOLUTE_TESTS)

        L_c = xp_reshape(xp_asarray(L_c, xp=xp), (2, 3), xp=xp)
        V_c = xp_reshape(xp_asarray(V_c, xp=xp), (2, 3), xp=xp)
        xp_assert_close(oetf_SMPTE240M(L_c), V_c, atol=TOLERANCE_ABSOLUTE_TESTS)

        L_c = xp_reshape(xp_asarray(L_c, xp=xp), (2, 3, 1), xp=xp)
        V_c = xp_reshape(xp_asarray(V_c, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(oetf_SMPTE240M(L_c), V_c, atol=TOLERANCE_ABSOLUTE_TESTS)
...
oetf_SMPTE240M` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    oetf_SMPTE240M(xp_asarray(L_c * factor, xp=xp)),
                    V_c * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
eotf_SMPTE240M` definition.
        ...
        xp_assert_close(
            eotf_SMPTE240M(xp_asarray(0.0, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_SMPTE240M(xp_asarray(0.080000000000000, xp=xp)),
            0.02,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_SMPTE240M(xp_asarray(0.402285796753870, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            eotf_SMPTE240M(xp_asarray(1.0, xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
eotf_SMPTE240M` definition n-dimensional arrays support.
        ...
        L_r = np.asarray(eotf_SMPTE240M(xp_asarray(V_r, xp=xp)))

        V_r = xp.tile(xp_asarray(V_r, xp=xp), (6,))
        L_r = xp.tile(xp_asarray(L_r, xp=xp), (6,))
        xp_assert_close(eotf_SMPTE240M(V_r), L_r, atol=TOLERANCE_ABSOLUTE_TESTS)

        V_r = xp_reshape(xp_asarray(V_r, xp=xp), (2, 3), xp=xp)
        L_r = xp_reshape(xp_asarray(L_r, xp=xp), (2, 3), xp=xp)
        xp_assert_close(eotf_SMPTE240M(V_r), L_r, atol=TOLERANCE_ABSOLUTE_TESTS)

        V_r = xp_reshape(xp_asarray(V_r, xp=xp), (2, 3, 1), xp=xp)
        L_r = xp_reshape(xp_asarray(L_r, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(eotf_SMPTE240M(V_r), L_r, atol=TOLERANCE_ABSOLUTE_TESTS)
...
eotf_SMPTE240M` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    eotf_SMPTE240M(xp_asarray(V_r * factor, xp=xp)),
                    L_r * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
