...
log_encoding_OPPOOLog` definition.
        ...
        xp_assert_close(
            log_encoding_OPPOOLog(xp_asarray(0.0, xp=xp)),
            0.06309903,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_OPPOOLog(xp_asarray(0.18, xp=xp)),
            0.38959139,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_encoding_OPPOOLog(xp_asarray(0.90, xp=xp)),
            0.60225879,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_encoding_OPPOOLog` definition n-dimensional arrays support.
        ...
        R = 0.18
        P = np.asarray(log_encoding_OPPOOLog(xp_asarray(R, xp=xp)))
...
log_encoding_OPPOOLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_encoding_OPPOOLog(xp_asarray(R * factor, xp=xp)),
                    P * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
...
log_decoding_OPPOOLog` definition.
        ...
        xp_assert_close(
            log_decoding_OPPOOLog(xp_asarray(0.06309903, xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_OPPOOLog(xp_asarray(0.38959139, xp=xp)),
            0.18,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            log_decoding_OPPOOLog(xp_asarray(0.60225879, xp=xp)),
            0.90,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
log_decoding_OPPOOLog` definition n-dimensional arrays support.
        ...
        P = 0.38959139
        R = np.asarray(log_decoding_OPPOOLog(xp_asarray(P, xp=xp)))
...
log_decoding_OPPOOLog` definition domain and range scale support.
        ...
        for scale, factor in d_r:
            with domain_range_scale(scale):
                xp_assert_close(
                    log_decoding_OPPOOLog(xp_asarray(P * factor, xp=xp)),
                    R * factor,
                    atol=TOLERANCE_ABSOLUTE_TESTS,
                )
