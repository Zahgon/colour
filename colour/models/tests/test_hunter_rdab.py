...
class TestXYZ_to_Hunter_Rdab:
    ...
    def test_n_dimensional_XYZ_to_Hunter_Rdab(self, xp: ModuleType) -> None:
        ...
        XYZ_n = D65.XYZ_n
        K_ab = D65.K_ab
        R_d_ab = np.asarray(XYZ_to_Hunter_Rdab(XYZ, XYZ_n, K_ab))

        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6, 1))
        R_d_ab = xp.tile(xp_asarray(R_d_ab, xp=xp), (6, 1))
        xp_assert_close(
        ...
        R_d_ab = xp_reshape(xp_asarray(R_d_ab, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(
            XYZ_to_Hunter_Rdab(XYZ, XYZ_n, K_ab),
            R_d_ab,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
class TestHunter_Rdab_to_XYZ:
    ...
    def test_Hunter_Rdab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Hunter_Rdab_to_XYZ(
                xp_asarray([12.19722500, 57.12537874, 17.46241341], xp=xp)
            ),
        ...
        xp_assert_close(
            Hunter_Rdab_to_XYZ(
                xp_asarray([23.04276800, -32.40057474, 20.96542183], xp=xp)
            ),
        ...
        xp_assert_close(
            Hunter_Rdab_to_XYZ(
                xp_asarray([6.15720100, 18.13400284, -67.14408607], xp=xp)
            ),
        ...
        xp_assert_close(
            Hunter_Rdab_to_XYZ(
                xp_asarray([12.19722500, 42.53572838, -3.00653110], xp=xp),
                A.XYZ_n,
                A.K_ab,
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]) * 100,
        ...
        xp_assert_close(
            Hunter_Rdab_to_XYZ(
                xp_asarray([12.19722500, 57.11906384, 17.45962317], xp=xp),
                D65.XYZ_n,
                K_ab=None,
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]) * 100,
    ...
    def test_n_dimensional_Hunter_Rdab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        R_d_ab = xp_asarray([12.19722500, 57.12537874, 17.46241341], xp=xp)
        XYZ_n = D65.XYZ_n
        K_ab = D65.K_ab
        XYZ = np.asarray(Hunter_Rdab_to_XYZ(R_d_ab, XYZ_n, K_ab))
