...
class TestXYZ_to_sUCS:
    ...
    def test_XYZ_to_sUCS(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_sUCS(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([42.62923653, 36.97646831, 14.12301358]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_sUCS(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([51.93649255, -18.89245582, 15.76112395]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_sUCS(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)),
...
class TestsUCS_to_XYZ:
    ...
    def test_sUCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            sUCS_to_XYZ(xp_asarray([42.62923653, 36.97646831, 14.12301358], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            sUCS_to_XYZ(xp_asarray([51.93649255, -18.89245582, 15.76112395], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            sUCS_to_XYZ(xp_asarray([29.79456846, -6.83806757, -25.33884097], xp=xp)),
    ...
    def test_n_dimensional_sUCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Iab = xp_asarray([42.62923653, 36.97646831, 14.12301358], xp=xp)
        XYZ = np.asarray(sUCS_to_XYZ(Iab))
...
class TestsUCSChroma:
    ...
    def test_sUCS_hue_angle(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            sUCS_chroma(xp_asarray([42.62923653, 36.97646831, 14.12301358], xp=xp)),
            40.420511061137226,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            sUCS_chroma(xp_asarray([51.93649255, -18.89245582, 15.76112395], xp=xp)),
            29.437831501432590,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            sUCS_chroma(xp_asarray([29.79456846, -6.83806757, -25.33884097], xp=xp)),
            30.800979756091614,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
class TestsUCSHueAngle:
    ...
    def test_sUCS_hue_angle(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            sUCS_hue_angle(xp_asarray([42.62923653, 36.97646831, 14.12301358], xp=xp)),
            20.904156072136217,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            sUCS_hue_angle(xp_asarray([51.93649255, -18.89245582, 15.76112395], xp=xp)),
            140.163281067124470,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            sUCS_hue_angle(xp_asarray([29.79456846, -6.83806757, -25.33884097], xp=xp)),
            254.897631851863850,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_sUCS_hue_angle(self, xp: ModuleType) -> None:
        ...
        Iab = xp_asarray([42.62923653, 36.97646831, 14.12301358], xp=xp)
        hue = np.asarray(sUCS_hue_angle(Iab))

        Iab = xp.tile(xp_asarray(Iab, xp=xp), (6, 1))
        hue = xp.tile(xp_asarray(hue, xp=xp), (6,))
        xp_assert_close(sUCS_hue_angle(Iab), hue, atol=TOLERANCE_ABSOLUTE_TESTS)

        Iab = xp_reshape(xp_asarray(Iab, xp=xp), (2, 3, 3), xp=xp)
        hue = xp_reshape(xp_asarray(hue, xp=xp), (2, 3), xp=xp)
        xp_assert_close(sUCS_hue_angle(Iab), hue, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestsUCS_Iab_to_sUCS_ICh:
    ...
    def test_sUCS_Iab_to_sUCS_ICh(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            sUCS_Iab_to_sUCS_ICh(
                xp_asarray([42.62923653, 36.97646831, 14.12301358], xp=xp)
            ),
        ...
        xp_assert_close(
            sUCS_Iab_to_sUCS_ICh(
                xp_asarray([51.93649255, -18.89245582, 15.76112395], xp=xp)
            ),
        ...
        xp_assert_close(
            sUCS_Iab_to_sUCS_ICh(
                xp_asarray([29.79456846, -6.83806757, -25.33884097], xp=xp)
            ),
    ...
    def test_n_dimensional_sUCS_Iab_to_sUCS_ICh(self, xp: ModuleType) -> None:
        ...
        Iab = xp_asarray([42.62923653, 36.97646831, 14.12301358], xp=xp)
        ICh = np.asarray(sUCS_Iab_to_sUCS_ICh(Iab))

        Iab = xp.tile(xp_asarray(Iab, xp=xp), (6, 1))
        ICh = xp.tile(xp_asarray(ICh, xp=xp), (6, 1))
        xp_assert_close(sUCS_Iab_to_sUCS_ICh(Iab), ICh, atol=TOLERANCE_ABSOLUTE_TESTS)

        Iab = xp_reshape(xp_asarray(Iab, xp=xp), (2, 3, 3), xp=xp)
        ICh = xp_reshape(xp_asarray(ICh, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(sUCS_Iab_to_sUCS_ICh(Iab), ICh, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestsUCS_ICh_to_sUCS_Iab:
    ...
    def test_sUCS_ICh_to_sUCS_Iab(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            sUCS_ICh_to_sUCS_Iab(
                xp_asarray([42.62923653, 40.42051106, 20.90415607], xp=xp)
            ),
            np.array([42.62923653, 36.97646831, 14.12301358]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            sUCS_ICh_to_sUCS_Iab(
                xp_asarray([51.93649255, 29.43783150, 140.16328107], xp=xp)
            ),
            np.array([51.93649255, -18.89245582, 15.76112395]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            sUCS_ICh_to_sUCS_Iab(
                xp_asarray([29.79456846, 30.80097976, 254.89763185], xp=xp)
            ),
            np.array([29.79456846, -6.83806757, -25.33884097]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_sUCS_ICh_to_sUCS_Iab(self, xp: ModuleType) -> None:
        ...
        ICh = xp_asarray([42.62923653, 40.42051106, 20.90415607], xp=xp)
        Iab = np.asarray(sUCS_ICh_to_sUCS_Iab(ICh))
