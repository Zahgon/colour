...
class TestExponent_hdr_IPT:
    ...
    def test_exponent_hdr_IPT(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            exponent_hdr_IPT(xp_asarray([0.2], xp=xp), xp_asarray([100], xp=xp)),
            np.array([0.482020919845900]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            exponent_hdr_IPT(xp_asarray([0.4], xp=xp), xp_asarray([100], xp=xp)),
            np.array([0.667413581325092]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            ...
            np.array([1.219933220992410]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            exponent_hdr_IPT(xp_asarray([0.2], xp=xp), xp_asarray([1000], xp=xp)),
            np.array([0.723031379768850]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_exponent_hdr_IPT(self, xp: ModuleType) -> None:
        ...
        Y_s = 0.2
        Y_abs = 100
        epsilon = np.asarray(exponent_hdr_IPT(Y_s, Y_abs))
...
class TestXYZ_to_hdr_IPT:
    ...
    def test_XYZ_to_hdr_IPT(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_hdr_IPT(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
        ...
        xp_assert_close(
            XYZ_to_hdr_IPT(
                xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp), Y_abs=1000
            ),
    ...
    def test_n_dimensional_XYZ_to_hdr_IPT(self, xp: ModuleType) -> None:
        ...
        Y_s = 0.2
        Y_abs = 100
        IPT_hdr = np.asarray(XYZ_to_hdr_IPT(XYZ, Y_s, Y_abs))

        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6, 1))
        IPT_hdr = xp.tile(xp_asarray(IPT_hdr, xp=xp), (6, 1))
        xp_assert_close(
        ...
        Y_s = xp_reshape(xp_asarray(Y_s, xp=xp), (2, 3), xp=xp)
        Y_abs = xp_reshape(xp_asarray(Y_abs, xp=xp), (2, 3), xp=xp)
        IPT_hdr = xp_reshape(xp_asarray(IPT_hdr, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(
            XYZ_to_hdr_IPT(XYZ, Y_s, Y_abs),
            IPT_hdr,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
class TestHdr_IPT_to_XYZ:
    ...
    def test_hdr_IPT_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            hdr_IPT_to_XYZ(xp_asarray([48.39376346, 42.44990202, 22.01954033], xp=xp)),
        ...
        xp_assert_close(
            hdr_IPT_to_XYZ(
                xp_asarray([30.02873147, 83.93845061, 34.90287382], xp=xp),
                method="Fairchild 2010",
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        xp_assert_close(
            hdr_IPT_to_XYZ(
                xp_asarray([20.75088680, 37.98300971, 16.66974299], xp=xp), Y_s=0.5
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            hdr_IPT_to_XYZ(
                xp_asarray([23.83205010, -5.98739209, -32.74311745], xp=xp), Y_abs=1000
            ),
    ...
    def test_n_dimensional_hdr_IPT_to_XYZ(self, xp: ModuleType) -> None:
        ...
        IPT_hdr = xp_asarray([48.39376346, 42.44990202, 22.01954033], xp=xp)
        Y_s = 0.2
        Y_abs = 100
        XYZ = np.asarray(hdr_IPT_to_XYZ(IPT_hdr, Y_s, Y_abs))
    ...
    def test_domain_range_scale_hdr_IPT_to_XYZ(self, xp: ModuleType) -> None:
        ...
        IPT_hdr = xp_asarray([24.88927680, -11.44574144, 1.63147707], xp=xp)
        Y_s = 0.2
        Y_abs = 100
        XYZ = np.asarray(hdr_IPT_to_XYZ(IPT_hdr, Y_s, Y_abs))
