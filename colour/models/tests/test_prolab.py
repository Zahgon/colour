...
class TestXYZ_to_ProLab:
    ...
    def test_XYZ_to_ProLab(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_ProLab(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([48.7948929, 35.31503175, 13.30044932]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_ProLab(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([64.45929636, -21.67007419, 13.25749056]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_ProLab(xp_asarray([0.96907232, 1.00000000, 0.12179215], xp=xp)),
...
class TestProLab_to_XYZ:
    ...
    def test_ProLab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            ProLab_to_XYZ(xp_asarray([48.7948929, 35.31503175, 13.30044932], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ProLab_to_XYZ(xp_asarray([64.45929636, -21.67007419, 13.25749056], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ProLab_to_XYZ(xp_asarray([100.0, 5.47367608, 37.26313098], xp=xp)),
    ...
    def test_n_dimensional_XYZ_to_ProLab(self, xp: ModuleType) -> None:
        ...
        ProLab = xp_asarray([48.7948929, 35.31503175, 13.30044932], xp=xp)
        XYZ = np.asarray(ProLab_to_XYZ(ProLab))
    ...
    def test_domain_range_scale_XYZ_to_ProLab(self, xp: ModuleType) -> None:
        ...
        ProLab = xp_asarray([48.7948929, 35.31503175, 13.30044932], xp=xp)
        XYZ = np.asarray(XYZ_to_ProLab(ProLab))
