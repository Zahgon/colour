...
class TestXYZ_to_UCS:
    ...
    def test_XYZ_to_UCS(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_UCS(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([0.13769339, 0.12197225, 0.10537310]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_UCS(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([0.09481340, 0.23042768, 0.32701033]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_UCS(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)),
    ...
    def test_n_dimensional_XYZ_to_UCS(self, xp: ModuleType) -> None:
        ...
        UCS = np.asarray(XYZ_to_UCS(XYZ))

        UCS = xp.tile(xp_asarray(UCS, xp=xp), (6, 1))
        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6, 1))
        xp_assert_close(XYZ_to_UCS(XYZ), UCS, atol=TOLERANCE_ABSOLUTE_TESTS)

        UCS = xp_reshape(xp_asarray(UCS, xp=xp), (2, 3, 3), xp=xp)
    ...
    def test_domain_range_scale_XYZ_to_UCS(self, xp: ModuleType) -> None:
        ...
        XYZ = xp_asarray([0.0704953400, 0.1008000000, 0.0955831300], xp=xp)
        UCS = np.asarray(XYZ_to_UCS(XYZ))
...
class TestUCS_to_XYZ:
    ...
    def test_UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            UCS_to_XYZ(xp_asarray([0.13769339, 0.12197225, 0.10537310], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UCS_to_XYZ(xp_asarray([0.09481340, 0.23042768, 0.32701033], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UCS_to_XYZ(xp_asarray([0.05212520, 0.06157201, 0.19376075], xp=xp)),
    ...
    def test_n_dimensional_UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        UCS = xp_asarray([0.13769339, 0.12197225, 0.10537310], xp=xp)
        XYZ = np.asarray(UCS_to_XYZ(UCS))
    ...
    def test_domain_range_scale_UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        UCS = xp_asarray([0.0469968933, 0.1008000000, 0.1637438950], xp=xp)
        XYZ = np.asarray(UCS_to_XYZ(UCS))
...
class TestUCS_to_uv:
    ...
    def test_UCS_to_uv(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            UCS_to_uv(xp_asarray([0.13769339, 0.12197225, 0.10537310], xp=xp)),
            np.array([0.37720213, 0.33413508]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UCS_to_uv(xp_asarray([0.09481340, 0.23042768, 0.32701033], xp=xp)),
            np.array([0.14536327, 0.35328046]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UCS_to_uv(xp_asarray([0.05212520, 0.06157201, 0.19376075], xp=xp)),
...
class Testuv_to_UCS:
    ...
    def test_uv_to_UCS(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            uv_to_UCS(xp_asarray([0.37720213, 0.33413508], xp=xp)),
            np.array([1.12889114, 1.00000000, 0.86391046]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            uv_to_UCS(xp_asarray([0.14536327, 0.35328046], xp=xp)),
            np.array([0.41146705, 1.00000000, 1.41914520]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            uv_to_UCS(xp_asarray([0.16953602, 0.20026156], xp=xp)),
            np.array([0.84657295, 1.00000000, 3.14689659]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            uv_to_UCS(xp_asarray([0.37720213, 0.33413508], xp=xp), V=0.18),
    ...
    def test_n_dimensional_uv_to_UCS(self, xp: ModuleType) -> None:
        ...
        uv = xp_asarray([0.37720213, 0.33413508], xp=xp)
        UCS = np.asarray(uv_to_UCS(uv))
...
class TestUCS_uv_to_xy:
    ...
    def test_UCS_uv_to_xy(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            UCS_uv_to_xy(xp_asarray([0.37720213, 0.33413508], xp=xp)),
            np.array([0.54369555, 0.32107941]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UCS_uv_to_xy(xp_asarray([0.14536327, 0.35328046], xp=xp)),
            np.array([0.29777734, 0.48246445]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UCS_uv_to_xy(xp_asarray([0.16953602, 0.20026156], xp=xp)),
...
class TestXy_to_UCS_uv:
    ...
    def test_xy_to_UCS_uv(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            xy_to_UCS_uv(xp_asarray([0.54369555, 0.32107941], xp=xp)),
            np.array([0.37720213, 0.33413508]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            xy_to_UCS_uv(xp_asarray([0.29777734, 0.48246445], xp=xp)),
            np.array([0.14536327, 0.35328046]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            xy_to_UCS_uv(xp_asarray([0.18582823, 0.14633764], xp=xp)),
            np.array([0.16953602, 0.20026156]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_xy_to_UCS_uv(self, xp: ModuleType) -> None:
        ...
        xy = xp_asarray([0.54369555, 0.32107941], xp=xp)
        uv = np.asarray(xy_to_UCS_uv(xy))
...
class TestXYZ_to_CIE1960UCS:
    ...
    def test_XYZ_to_CIE1960UCS(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_CIE1960UCS(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([0.37720213, 0.33413509, 0.12197225]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_CIE1960UCS(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([0.14536327, 0.35328046, 0.23042768]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_CIE1960UCS(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)),
    ...
    def test_n_dimensional_XYZ_to_CIE1960UCS(self, xp: ModuleType) -> None:
        ...
        XYZ = xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)
        uvV = np.asarray(XYZ_to_CIE1960UCS(XYZ))

        uvV = xp.tile(xp_asarray(uvV, xp=xp), (6, 1))
        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6, 1))
        xp_assert_close(XYZ_to_CIE1960UCS(XYZ), uvV, atol=TOLERANCE_ABSOLUTE_TESTS)

        uvV = xp_reshape(xp_asarray(uvV, xp=xp), (2, 3, 3), xp=xp)
        XYZ = xp_reshape(xp_asarray(XYZ, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(XYZ_to_CIE1960UCS(XYZ), uvV, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestCIE1960UCS_to_XYZ:
    ...
    def test_CIE1960UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            CIE1960UCS_to_XYZ(xp_asarray([0.37720213, 0.33413509, 0.12197225], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            CIE1960UCS_to_XYZ(xp_asarray([0.14536327, 0.35328046, 0.23042768], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            CIE1960UCS_to_XYZ(xp_asarray([0.16953603, 0.20026156, 0.06157201], xp=xp)),
            np.array([0.07818780, 0.06157201, 0.28099326]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_CIE1960UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        uvV = xp_asarray([0.37720213, 0.33413509, 0.12197225], xp=xp)
        XYZ = np.asarray(CIE1960UCS_to_XYZ(uvV))
    ...
    def test_domain_range_scale_CIE1960UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        uvV = xp_asarray([0.0469968933, 0.1008000000, 0.1637438950], xp=xp)
        XYZ = np.asarray(CIE1960UCS_to_XYZ(uvV))
