...
class TestLMS_to_Yrg:
    ...
    def test_LMS_to_Yrg(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            LMS_to_Yrg(xp_asarray([0.15639195, 0.06741689, 0.03281398], xp=xp)),
            np.array([0.13137801, 0.49037644, 0.37777391]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            LMS_to_Yrg(xp_asarray([0.23145723, 0.22601133, 0.05033211], xp=xp)),
            np.array([0.23840767, 0.20110504, 0.69668437]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            LMS_to_Yrg(xp_asarray([1.07423297, 0.91295620, 0.61375713], xp=xp)),
    ...
    def test_n_dimensional_LMS_to_Yrg(self, xp: ModuleType) -> None:
        ...
        LMS = xp_asarray([0.15639195, 0.06741689, 0.03281398], xp=xp)
        Yrg = np.asarray(LMS_to_Yrg(LMS))

        LMS = xp.tile(xp_asarray(LMS, xp=xp), (6, 1))
        Yrg = xp.tile(xp_asarray(Yrg, xp=xp), (6, 1))
        xp_assert_close(LMS_to_Yrg(LMS), Yrg, atol=TOLERANCE_ABSOLUTE_TESTS)

        LMS = xp_reshape(xp_asarray(LMS, xp=xp), (2, 3, 3), xp=xp)
        Yrg = xp_reshape(xp_asarray(Yrg, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(LMS_to_Yrg(LMS), Yrg, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestYrg_to_LMS:
    ...
    def test_Yrg_to_LMS(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Yrg_to_LMS(xp_asarray([0.13137801, 0.49037644, 0.37777391], xp=xp)),
            np.array([0.15639195, 0.06741689, 0.03281398]),
            atol=TOLERANCE_ABSOLUTE_TESTS * 1000,
        )

        xp_assert_close(
            Yrg_to_LMS(xp_asarray([0.23840767, 0.20110504, 0.69668437], xp=xp)),
            np.array([0.23145723, 0.22601133, 0.05033211]),
            atol=TOLERANCE_ABSOLUTE_TESTS * 1000,
        )

        xp_assert_close(
            Yrg_to_LMS(xp_asarray([1.05911888, 0.22010094, 0.53660290], xp=xp)),
    ...
    def test_n_dimensional_Yrg_to_LMS(self, xp: ModuleType) -> None:
        ...
        Yrg = xp_asarray([0.00535048, 0.00924302, 0.00526007], xp=xp)
        LMS = np.asarray(Yrg_to_LMS(Yrg))
...
class TestXYZ_to_Yrg:
    ...
    def test_XYZ_to_Yrg(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_Yrg(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([0.13137801, 0.49037645, 0.37777388]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_Yrg(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([0.23840767, 0.20110503, 0.69668437]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_Yrg(xp_asarray([0.96907232, 1.00000000, 1.12179215], xp=xp)),
            np.array([1.05911888, 0.22010094, 0.53660290]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
class TestYrg_to_XYZ:
    ...
    def test_Yrg_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Yrg_to_XYZ(xp_asarray([0.13137801, 0.49037645, 0.37777388], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS * 1000,
        )

        xp_assert_close(
            Yrg_to_XYZ(xp_asarray([0.23840767, 0.20110503, 0.69668437], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS * 2000,
        )

        xp_assert_close(
            Yrg_to_XYZ(xp_asarray([1.05911888, 0.22010094, 0.53660290], xp=xp)),
    ...
    def test_n_dimensional_Yrg_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Yrg = xp_asarray([0.13137801, 0.49037645, 0.37777388], xp=xp)
        XYZ = np.asarray(Yrg_to_XYZ(Yrg))
