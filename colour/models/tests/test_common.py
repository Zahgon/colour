...
class TestJab_to_JCh:
    ...
    def test_Jab_to_JCh(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Jab_to_JCh(xp_asarray([41.52787529, 52.63858304, 26.92317922], xp=xp)),
            np.array([41.52787529, 59.12425901, 27.08848784]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Jab_to_JCh(xp_asarray([55.11636304, -41.08791787, 30.91825778], xp=xp)),
            np.array([55.11636304, 51.42135412, 143.03889556]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Jab_to_JCh(xp_asarray([29.80565520, 20.01830466, -48.34913874], xp=xp)),
...
class TestJCh_to_Jab:
    ...
    def test_JCh_to_Jab(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            JCh_to_Jab(xp_asarray([41.52787529, 59.12425901, 27.08848784], xp=xp)),
            np.array([41.52787529, 52.63858304, 26.92317922]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            JCh_to_Jab(xp_asarray([55.11636304, 51.42135412, 143.03889556], xp=xp)),
            np.array([55.11636304, -41.08791787, 30.91825778]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            JCh_to_Jab(xp_asarray([29.80565520, 52.32945383, 292.49133666], xp=xp)),
    ...
    def test_n_dimensional_JCh_to_Jab(self, xp: ModuleType) -> None:
        ...
        LCHab = xp_asarray([41.52787529, 59.12425901, 27.08848784], xp=xp)
        Lab = np.asarray(JCh_to_Jab(LCHab))
...
class TestIab_to_XYZ:
    ...
    def test_Iab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Iab_to_XYZ(
                xp_asarray([0.38426191, 0.38487306, 0.18886838], xp=xp),
                self.LMS_p_to_LMS,
                self.M_Iab_to_LMS_p,
                self.M_LMS_to_XYZ,
            ),
        ...
        xp_assert_close(
            Iab_to_XYZ(
                xp_asarray([0.49437481, -0.19251742, 0.18080304], xp=xp),
                self.LMS_p_to_LMS,
                self.M_Iab_to_LMS_p,
                self.M_LMS_to_XYZ,
            ),
        ...
        xp_assert_close(
            Iab_to_XYZ(
                xp_asarray([0.35167774, -0.07525627, -0.30921279], xp=xp),
                self.LMS_p_to_LMS,
                self.M_Iab_to_LMS_p,
                self.M_LMS_to_XYZ,
            ),
    ...
    def test_n_dimensional_Iab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Iab = xp_asarray([0.38426191, 0.38487306, 0.18886838], xp=xp)
        XYZ = np.asarray(
