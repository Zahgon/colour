...
class TestExponent_hdr_CIELab:
    ...
    def test_exponent_hdr_CIELab(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            exponent_hdr_CIELab(xp_asarray([0.2], xp=xp), xp_asarray([100], xp=xp)),
            np.array([0.473851073746817]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            exponent_hdr_CIELab(xp_asarray([0.4], xp=xp), xp_asarray([100], xp=xp)),
            np.array([0.656101486726362]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            exponent_hdr_CIELab(
                xp_asarray([0.4], xp=xp),
                xp_asarray([100], xp=xp),
                method="Fairchild 2010",
            ),
            np.array([1.326014370643925]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            exponent_hdr_CIELab(xp_asarray([0.2], xp=xp), xp_asarray([1000], xp=xp)),
            np.array([0.710776610620225]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_exponent_hdr_CIELab(self, xp: ModuleType) -> None:
        ...
        Y_s = 0.2
        Y_abs = 100
        epsilon = np.asarray(exponent_hdr_CIELab(Y_s, Y_abs))

        Y_s = xp.tile(xp_asarray(Y_s, xp=xp), (6,))
        Y_abs = xp.tile(xp_asarray(Y_abs, xp=xp), (6,))
        epsilon = xp.tile(xp_asarray(epsilon, xp=xp), (6,))
        xp_assert_close(
        ...
        Y_s = xp_reshape(xp_asarray(Y_s, xp=xp), (2, 3), xp=xp)
        Y_abs = xp_reshape(xp_asarray(Y_abs, xp=xp), (2, 3), xp=xp)
        epsilon = xp_reshape(xp_asarray(epsilon, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            exponent_hdr_CIELab(Y_s, Y_abs),
            epsilon,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        Y_s = xp_reshape(xp_asarray(Y_s, xp=xp), (2, 3, 1), xp=xp)
        Y_abs = xp_reshape(xp_asarray(Y_abs, xp=xp), (2, 3, 1), xp=xp)
        epsilon = xp_reshape(xp_asarray(epsilon, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(
            exponent_hdr_CIELab(Y_s, Y_abs),
            epsilon,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
class TestXYZ_to_hdr_CIELab:
    ...
    def test_XYZ_to_hdr_CIELab(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_hdr_CIELab(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
        ...
        xp_assert_close(
            XYZ_to_hdr_CIELab(
                xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp), Y_s=0.5
            ),
        ...
        xp_assert_close(
            XYZ_to_hdr_CIELab(
                xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp), Y_abs=1000
            ),
    ...
    def test_n_dimensional_XYZ_to_hdr_CIELab(self, xp: ModuleType) -> None:
        ...
        Y_s = 0.2
        Y_abs = 100
        Lab_hdr = np.asarray(XYZ_to_hdr_CIELab(XYZ, illuminant, Y_s, Y_abs))

        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6, 1))
        Lab_hdr = xp.tile(xp_asarray(Lab_hdr, xp=xp), (6, 1))
        xp_assert_close(
        ...
        Y_s = xp_reshape(xp_asarray(Y_s, xp=xp), (2, 3), xp=xp)
        Y_abs = xp_reshape(xp_asarray(Y_abs, xp=xp), (2, 3), xp=xp)
        Lab_hdr = xp_reshape(xp_asarray(Lab_hdr, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(
            XYZ_to_hdr_CIELab(XYZ, illuminant, Y_s, Y_abs),
            Lab_hdr,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
class TestHdr_CIELab_to_XYZ:
    ...
    def test_hdr_CIELab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            hdr_CIELab_to_XYZ(
                xp_asarray([51.87002062, 60.47633850, 32.14551912], xp=xp)
            ),
        ...
        xp_assert_close(
            hdr_CIELab_to_XYZ(
                xp_asarray([51.87002062, 44.49667330, -6.69619196], xp=xp),
                xp_asarray([0.44757, 0.40745], xp=xp),
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        xp_assert_close(
            hdr_CIELab_to_XYZ(
                xp_asarray([31.99621114, 95.08564341, -14.14047055], xp=xp),
                xp_asarray([0.44757, 0.40745], xp=xp),
                method="Fairchild 2010",
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
        ...
        xp_assert_close(
            hdr_CIELab_to_XYZ(
                xp_asarray([23.10388654, 59.31425004, 23.69960142], xp=xp), Y_s=0.5
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            hdr_CIELab_to_XYZ(
                xp_asarray([29.77261805, 62.58315675, 27.31232673], xp=xp), Y_abs=1000
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_hdr_CIELab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Lab_hdr = xp_asarray([51.87002062, 60.47633850, 32.14551912], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        Y_s = 0.2
        Y_abs = 100
        XYZ = np.asarray(hdr_CIELab_to_XYZ(Lab_hdr, illuminant, Y_s, Y_abs))
    ...
    def test_domain_range_scale_hdr_CIELab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Lab_hdr = xp_asarray([26.46461067, -24.61332600, -4.84796811], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        Y_s = 0.2
        Y_abs = 100
        XYZ = np.asarray(hdr_CIELab_to_XYZ(Lab_hdr, illuminant, Y_s, Y_abs))
