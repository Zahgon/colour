...
class TestXYZ_to_IPT:
    ...
    def test_XYZ_to_IPT(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_IPT(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([0.38426191, 0.38487306, 0.18886838]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_IPT(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([0.49437481, -0.19251742, 0.18080304]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_IPT(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)),
...
class TestIPT_to_XYZ:
    ...
    def test_IPT_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            IPT_to_XYZ(xp_asarray([0.38426191, 0.38487306, 0.18886838], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            IPT_to_XYZ(xp_asarray([0.49437481, -0.19251742, 0.18080304], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            IPT_to_XYZ(xp_asarray([0.35167774, -0.07525627, -0.30921279], xp=xp)),
    ...
    def test_n_dimensional_IPT_to_XYZ(self, xp: ModuleType) -> None:
        ...
        IPT = xp_asarray([0.38426191, 0.38487306, 0.18886838], xp=xp)
        XYZ = np.asarray(IPT_to_XYZ(IPT))
...
class TestIPTHueAngle:
    ...
    def test_IPT_hue_angle(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            IPT_hue_angle(xp_asarray([0.38426191, 0.38487306, 0.18886838], xp=xp)),
            26.138526939899490,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            IPT_hue_angle(xp_asarray([0.49437481, -0.19251742, 0.18080304], xp=xp)),
            136.797287973958500,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            IPT_hue_angle(xp_asarray([0.35167774, -0.07525627, -0.30921279], xp=xp)),
            256.321284526533300,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_IPT_hue_angle(self, xp: ModuleType) -> None:
        ...
        IPT = xp_asarray([0.38426191, 0.38487306, 0.18886838], xp=xp)
        hue = np.asarray(IPT_hue_angle(IPT))
