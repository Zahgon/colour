...
class TestXYZ_to_Oklab:
    ...
    def test_XYZ_to_Oklab(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_Oklab(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([0.51634019, 0.15469500, 0.06289579]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_Oklab(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([0.59910746, -0.11139207, 0.07508465]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_Oklab(xp_asarray([0.96907232, 1.00000000, 1.12179215], xp=xp)),
...
class TestOklab_to_XYZ:
    ...
    def test_Oklab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Oklab_to_XYZ(xp_asarray([0.51634019, 0.15469500, 0.06289579], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS * 10,
        )

        xp_assert_close(
            Oklab_to_XYZ(xp_asarray([0.59910746, -0.11139207, 0.07508465], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS * 10,
        )

        xp_assert_close(
            Oklab_to_XYZ(xp_asarray([1.00121561, 0.00899591, -0.00535107], xp=xp)),
    ...
    def test_n_dimensional_Oklab_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Oklab = xp_asarray([0.51634019, 0.15469500, 0.06289579], xp=xp)
        XYZ = np.asarray(Oklab_to_XYZ(Oklab))
