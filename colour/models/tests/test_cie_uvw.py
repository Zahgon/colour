...
class TestXYZ_to_UVW:
    ...
    def test_XYZ_to_UVW(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_UVW(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp) * 100),
            np.array([94.55035725, 11.55536523, 40.54757405]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_UVW(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp) * 100),
            np.array([-36.92762376, 28.90425105, 54.14071478]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_UVW(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp) * 100),
    ...
    def test_n_dimensional_XYZ_to_UVW(self, xp: ModuleType) -> None:
        ...
        UVW = np.asarray(XYZ_to_UVW(XYZ, illuminant))

        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6, 1))
        UVW = xp.tile(xp_asarray(UVW, xp=xp), (6, 1))
        ...
        UVW = xp_reshape(xp_asarray(UVW, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(XYZ_to_UVW(XYZ, illuminant), UVW, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestUVW_to_XYZ:
    ...
    def test_UVW_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            UVW_to_XYZ(xp_asarray([94.55035725, 11.55536523, 40.54757405], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]) * 100,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UVW_to_XYZ(xp_asarray([-36.92762376, 28.90425105, 54.14071478], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]) * 100,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            UVW_to_XYZ(xp_asarray([-10.60111550, -41.94580000, 28.82134002], xp=xp)),
        ...
        xp_assert_close(
            UVW_to_XYZ(
                xp_asarray([63.90676310, -8.11466183, 40.54757405], xp=xp),
                xp_asarray([0.44757, 0.40745], xp=xp),
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]) * 100,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        xp_assert_close(
            UVW_to_XYZ(
                xp_asarray([88.56798946, 4.61154385, 40.54757405], xp=xp),
                xp_asarray([0.34570, 0.35850], xp=xp),
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]) * 100,
            atol=TOLERANCE_ABSOLUTE_TESTS,
    ...
    def test_n_dimensional_UVW_to_XYZ(self, xp: ModuleType) -> None:
        ...
        UVW = xp_asarray([94.55035725, 11.55536523, 40.54757405], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        XYZ = np.asarray(UVW_to_XYZ(UVW, illuminant))
