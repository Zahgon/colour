...
class TestXYZ_to_ICaCb:
    ...
    def test_XYZ_to_ICaCb(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_ICaCb(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([0.06875297, 0.05753352, 0.02081548]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_ICaCb(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([0.08666353, -0.02479011, 0.03099396]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_ICaCb(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)),
            np.array([0.05102472, -0.00965461, -0.05150706]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_ICaCb(xp_asarray([0.00000000, 0.00000000, 1.00000000], xp=xp)),
    ...
    def test_domain_range_scale_XYZ_to_ICaCb(self, xp: ModuleType) -> None:
        ...
        XYZ = xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)
        ICaCb = np.asarray(XYZ_to_ICaCb(XYZ))
...
class TestICaCb_to_XYZ:
    ...
    def test_XYZ_to_ICaCb(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            ICaCb_to_XYZ(xp_asarray([0.06875297, 0.05753352, 0.02081548], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ICaCb_to_XYZ(xp_asarray([0.08666353, -0.02479011, 0.03099396], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            ICaCb_to_XYZ(xp_asarray([0.05102472, -0.00965461, -0.05150706], xp=xp)),
        ...
        xp_assert_close(
            ICaCb_to_XYZ(
                xp_asarray([1702.0656419, 14738.00583456, 1239.66837927], xp=xp)
            ),
    ...
    def test_n_dimensional_ICaCb_to_XYZ(self, xp: ModuleType) -> None:
        ...
        ICaCb = xp_asarray([0.06875297, 0.05753352, 0.02081548], xp=xp)
        XYZ = np.asarray(ICaCb_to_XYZ(ICaCb))
