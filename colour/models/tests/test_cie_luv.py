...
class TestXYZ_to_Luv:
    ...
    def test_XYZ_to_Luv(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_Luv(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([41.52787529, 96.83626054, 17.75210149]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_Luv(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([55.11636304, -37.59308176, 44.13768458]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_Luv(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)),
    ...
    def test_n_dimensional_XYZ_to_Luv(self, xp: ModuleType) -> None:
        ...
        Luv = np.asarray(XYZ_to_Luv(XYZ, illuminant))

        XYZ = xp.tile(xp_asarray(XYZ, xp=xp), (6, 1))
        Luv = xp.tile(xp_asarray(Luv, xp=xp), (6, 1))
        ...
        Luv = xp_reshape(xp_asarray(Luv, xp=xp), (2, 3, 3), xp=xp)
        xp_assert_close(XYZ_to_Luv(XYZ, illuminant), Luv, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestLuv_to_XYZ:
    ...
    def test_Luv_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Luv_to_XYZ(xp_asarray([41.52787529, 96.83626054, 17.75210149], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Luv_to_XYZ(xp_asarray([55.11636304, -37.59308176, 44.13768458], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Luv_to_XYZ(xp_asarray([29.80565520, -10.96316802, -65.06751860], xp=xp)),
        ...
        xp_assert_close(
            Luv_to_XYZ(
                xp_asarray([41.52787529, 65.45180940, -12.46626977], xp=xp),
                xp_asarray([0.44757, 0.40745], xp=xp),
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        xp_assert_close(
            Luv_to_XYZ(
                xp_asarray([41.52787529, 90.70925962, 7.08455273], xp=xp),
                xp_asarray([0.34570, 0.35850], xp=xp),
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
    ...
    def test_n_dimensional_Luv_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Luv = xp_asarray([41.52787529, 96.83626054, 17.75210149], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        XYZ = np.asarray(Luv_to_XYZ(Luv, illuminant))
...
class TestLuv_to_uv:
    ...
    def test_Luv_to_uv(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Luv_to_uv(xp_asarray([41.52787529, 96.83626054, 17.75210149], xp=xp)),
            np.array([0.37720213, 0.50120264]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Luv_to_uv(xp_asarray([55.11636304, -37.59308176, 44.13768458], xp=xp)),
            np.array([0.14536327, 0.52992069]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Luv_to_uv(xp_asarray([29.80565520, -10.96316802, -65.06751860], xp=xp)),
    ...
    def test_n_dimensional_Luv_to_uv(self, xp: ModuleType) -> None:
        ...
        Luv = xp_asarray([41.52787529, 96.83626054, 17.75210149], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        uv = np.asarray(Luv_to_uv(Luv, illuminant))
...
class Testuv_to_Luv:
    ...
    def test_uv_to_Luv(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            uv_to_Luv(xp_asarray([0.37720213, 0.50120264], xp=xp)),
            np.array([100.00000000, 233.18376036, 42.74743858]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            uv_to_Luv(xp_asarray([0.14536327, 0.52992069], xp=xp)),
            np.array([100.00000000, -68.20675764, 80.08090358]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            uv_to_Luv(xp_asarray([0.16953603, 0.30039234], xp=xp)),
        ...
        xp_assert_close(
            uv_to_Luv(
                xp_asarray([0.37720213, 0.50120264], xp=xp),
                xp_asarray([0.44757, 0.40745], xp=xp),
            ),
        ...
        xp_assert_close(
            uv_to_Luv(xp_asarray([0.37720213, 0.50120264], xp=xp), L=41.5278752),
            np.array([41.52787529, 96.83626054, 17.75210149]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_uv_to_Luv(self, xp: ModuleType) -> None:
        ...
        uv = xp_asarray([0.37720213, 0.50120264], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        Luv = np.asarray(uv_to_Luv(uv, illuminant))
    ...
    def test_domain_range_scale_uv_to_Luv(self, xp: ModuleType) -> None:
        ...
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        L = 100
        Luv = np.asarray(uv_to_Luv(uv, illuminant, L))
...
class TestLuv_uv_to_xy:
    ...
    def test_Luv_uv_to_xy(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            Luv_uv_to_xy(xp_asarray([0.37720213, 0.50120264], xp=xp)),
            np.array([0.54369558, 0.32107944]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Luv_uv_to_xy(xp_asarray([0.14536327, 0.52992069], xp=xp)),
            np.array([0.29777734, 0.48246445]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            Luv_uv_to_xy(xp_asarray([0.16953603, 0.30039234], xp=xp)),
...
class TestXy_to_Luv_uv:
    ...
    def test_xy_to_Luv_uv(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            xy_to_Luv_uv(xp_asarray([0.54369558, 0.32107944], xp=xp)),
            np.array([0.37720213, 0.50120264]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            xy_to_Luv_uv(xp_asarray([0.29777734, 0.48246445], xp=xp)),
            np.array([0.14536327, 0.52992069]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            xy_to_Luv_uv(xp_asarray([0.18582824, 0.14633764], xp=xp)),
            np.array([0.16953603, 0.30039234]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_xy_to_Luv_uv(self, xp: ModuleType) -> None:
        ...
        xy = xp_asarray([0.54369558, 0.32107944], xp=xp)
        uv = np.asarray(xy_to_Luv_uv(xy))
...
class TestXYZ_to_CIE1976UCS:
    ...
    def test_XYZ_to_CIE1976UCS(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            XYZ_to_CIE1976UCS(xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)),
            np.array([0.37720213, 0.50120264, 41.52787529]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_CIE1976UCS(xp_asarray([0.14222010, 0.23042768, 0.10495772], xp=xp)),
            np.array([0.14536327, 0.52992069, 55.11636304]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            XYZ_to_CIE1976UCS(xp_asarray([0.07818780, 0.06157201, 0.28099326], xp=xp)),
    ...
    def test_n_dimensional_XYZ_to_CIE1976UCS(self, xp: ModuleType) -> None:
        ...
        XYZ = xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        Luv = np.asarray(XYZ_to_CIE1976UCS(XYZ, illuminant))
    ...
    def test_domain_range_scale_XYZ_to_CIE1976UCS(self, xp: ModuleType) -> None:
        ...
        XYZ = xp_asarray([0.20654008, 0.12197225, 0.05136952], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        uvL = np.asarray(XYZ_to_CIE1976UCS(XYZ, illuminant))
...
class TestCIE1976UCS_to_XYZ:
    ...
    def test_CIE1976UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            CIE1976UCS_to_XYZ(xp_asarray([0.37720213, 0.50120264, 41.52787529], xp=xp)),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            CIE1976UCS_to_XYZ(xp_asarray([0.14536327, 0.52992069, 55.11636304], xp=xp)),
            np.array([0.14222010, 0.23042768, 0.10495772]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            CIE1976UCS_to_XYZ(xp_asarray([0.16953603, 0.30039234, 29.80565520], xp=xp)),
            np.array([0.07818780, 0.06157201, 0.28099326]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            CIE1976UCS_to_XYZ(
                xp_asarray([0.37720213, 0.50120264, 41.52787529], xp=xp),
                xp_asarray([0.44757, 0.40745], xp=xp),
            ),
            np.array([0.20654008, 0.12197225, 0.05136952]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
    ...
    def test_n_dimensional_CIE1976UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        Luv = xp_asarray([0.37720213, 0.50120264, 41.52787529], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        XYZ = np.asarray(CIE1976UCS_to_XYZ(Luv, illuminant))
    ...
    def test_domain_range_scale_CIE1976UCS_to_XYZ(self, xp: ModuleType) -> None:
        ...
        uvL = xp_asarray([0.37720213, 0.50120264, 41.52787529], xp=xp)
        illuminant = xp_asarray([0.31270, 0.32900], xp=xp)
        XYZ = np.asarray(CIE1976UCS_to_XYZ(uvL, illuminant))
