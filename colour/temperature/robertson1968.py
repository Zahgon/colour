...
def uv_to_CCT_Robertson1968(uv: ArrayLike) -> NDArrayFloat:
    ...
    r_itl, u_itl, v_itl, t_itl = tsplit(
        xp_asarray(DATA_ISOTEMPERATURE_LINES_ROBERTSON1968, xp=xp)
    )

    # Normalized direction vectors
    length = xp.hypot(xp_asarray(1.0, xp=xp), t_itl)
    du_itl = 1.0 / length
    dv_itl = t_itl / length
    ...
    mask = xp_astype(dt <= 0, DTYPE_INT_DEFAULT, xp)
...
def CCT_to_uv_Robertson1968(CCT_D_uv: ArrayLike) -> NDArrayFloat:
    ...
    mask = xp_astype(r[:, None] < r_itl[1:], DTYPE_INT_DEFAULT, xp)
