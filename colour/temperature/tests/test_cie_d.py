...
class TestXy_to_CCT_CIE_D:
    ...
    def test_xy_to_CCT_CIE_D(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            xy_to_CCT_CIE_D(
                xp_asarray([0.382343625000000, 0.383766261015578], xp=xp),
                {"method": "Nelder-Mead"},
            ),
            4000,
        ...
        xp_assert_close(
            xy_to_CCT_CIE_D(
                xp_asarray([0.305357431486880, 0.321646345474552], xp=xp),
                {"method": "Nelder-Mead"},
            ),
            7000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        xp_assert_close(
            xy_to_CCT_CIE_D(
                xp_asarray([0.24985367, 0.254799464210944], xp=xp),
                {"method": "Nelder-Mead"},
            ),
            25000,
            atol=TOLERANCE_ABSOLUTE_TESTS,
    ...
    def test_n_dimensional_xy_to_CCT_CIE_D(self, xp: ModuleType) -> None:
        ...
        xy = xp_asarray([0.382343625000000, 0.383766261015578], xp=xp)
        CCT = np.asarray(xy_to_CCT_CIE_D(xy))
...
class TestCCT_to_xy_CIE_D:
    ...
    def test_CCT_to_xy_CIE_D(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            CCT_to_xy_CIE_D(xp_asarray([4000], xp=xp)),
            np.array([[0.382343625000000, 0.383766261015578]]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            CCT_to_xy_CIE_D(xp_asarray([7000], xp=xp)),
            np.array([[0.305357431486880, 0.321646345474552]]),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            CCT_to_xy_CIE_D(xp_asarray([25000], xp=xp)),
    ...
    def test_n_dimensional_CCT_to_xy_CIE_D(self, xp: ModuleType) -> None:
        ...
        CCT = 4000
        xy = np.asarray(CCT_to_xy_CIE_D(CCT))
