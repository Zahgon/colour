...
class TestSignal:
    ...
    def test_dtype(self, xp: ModuleType) -> None:
        ...
        signal = Signal(xp_asarray(self._range, xp=xp))
    ...
    def test_domain(self, xp: ModuleType) -> None:
        ...
        signal.domain = xp_asarray(np.arange(0, 10, 1) * 10, xp=xp)
    ...
    def test_range(self, xp: ModuleType) -> None:
        ...
        signal.range = xp_asarray(self._range * 10, xp=xp)
    ...
    def test_extrapolator_kwargs(self, xp: ModuleType) -> None:
        ...
        attest(np.all(np.isnan(np.asarray(signal[np.array([-1000, 1000])]))))
    ...
    def test__init__(self, xp: ModuleType) -> None:
        ...
        signal = Signal(xp_asarray(self._range, xp=xp), xp_asarray(self._domain, xp=xp))
    ...
    def test__getitem__(self, xp: ModuleType) -> None:
        ...
        xp_assert_equal(signal[np.array([-1000, 1000])], np.array([-9990.0, 10010.0]))
        ...
        xp_assert_equal(signal[np.array([-1000, 1000])], np.array([0.0, 1.0]))
    ...
    def test__eq__(self, xp: ModuleType) -> None:
        ...
        signal_1 = Signal(xp_asarray(self._range, xp=xp))
        signal_2 = Signal(xp_asarray(self._range, xp=xp))
    ...
    def test_arithmetical_operation(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            ...
            np.asarray(signal.range) + self._range,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            signal.arithmetical_operation(signal, "+", False).range,
            np.asarray(signal.range) + np.asarray(signal.range),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_signal_unpack_data(self, xp: ModuleType) -> None:
        ...
        domain, range_ = Signal.signal_unpack_data(xp_asarray(self._range, xp=xp))
        xp_assert_equal(range_, self._range)
        xp_assert_equal(domain, np.arange(0, 10, 1))

        domain, range_ = Signal.signal_unpack_data(
            xp_asarray(self._range, xp=xp), xp_asarray(self._domain, xp=xp)
        )
        xp_assert_equal(range_, self._range)
        ...
        domain, range_ = Signal.signal_unpack_data(
            Signal(xp_asarray(self._range, xp=xp), xp_asarray(self._domain, xp=xp))
        )
        xp_assert_equal(range_, self._range)
        xp_assert_equal(domain, self._domain)
