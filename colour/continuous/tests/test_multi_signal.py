...
class TestMultiSignals:
    ...
    def test_dtype(self, xp: ModuleType) -> None:
        ...
        multi_signals = MultiSignals(xp_asarray(self._range_2, xp=xp))
    ...
    def test_domain(self, xp: ModuleType) -> None:
        ...
        multi_signals.domain = xp_asarray(self._domain_1 * 10, xp=xp)
    ...
    def test_range(self, xp: ModuleType) -> None:
        ...
        multi_signals.range = xp_asarray(self._range_1 * 10, xp=xp)
        ...
        multi_signals.range = xp_asarray(self._range_2 * 10, xp=xp)
...
extrapolator_kwargs` property.
        ...
        attest(np.all(np.isnan(np.asarray(multi_signals[np.array([-1000, 1000])]))))
...
function` property raised exception.
    ...
    def test_signals(self, xp: ModuleType) -> None:
        ...
        multi_signals.signals = xp_asarray(self._range_1, xp=xp)
    ...
    def test__init__(self, xp: ModuleType) -> None:
        ...
        multi_signals = MultiSignals(xp_asarray(self._range_1, xp=xp))
        xp_assert_equal(multi_signals.domain, self._domain_1)
        xp_assert_equal(multi_signals.range, self._range_1[:, None])

        multi_signals = MultiSignals(
            xp_asarray(self._range_1, xp=xp), xp_asarray(self._domain_2, xp=xp)
        )
        xp_assert_equal(multi_signals.domain, self._domain_2)
        xp_assert_equal(multi_signals.range, self._range_1[:, None])

        multi_signals = MultiSignals(
            xp_asarray(self._range_2, xp=xp), xp_asarray(self._domain_2, xp=xp)
        )
        xp_assert_equal(multi_signals.domain, self._domain_2)
        ...
        multi_signals = MultiSignals(
            xp_asarray(self._range_1, xp=xp), signal_type=NotSignal
        )
    ...
    def test__getitem__(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            multi_signals[:, 0:2],  # pyright: ignore
            np.array(
                [
                    [10.0, 20.0],
                    [20.0, 30.0],
    ...
    def test__setitem__(self, xp: ModuleType) -> None:
        ...
        multi_signals[np.array([0, 1, 2])] = xp_reshape(
            xp_asarray(np.arange(1, 10, 1), xp=xp),
            (3, 3),
            xp=xp,
        )
        xp_assert_close(
    ...
    def test__eq__(self, xp: ModuleType) -> None:
        ...
        signal_1 = MultiSignals(xp_asarray(self._range_2, xp=xp))
        signal_2 = MultiSignals(xp_asarray(self._range_2, xp=xp))
    ...
    def test__ne__(self, xp: ModuleType) -> None:
        ...
        multi_signals_1 = MultiSignals(xp_asarray(self._range_2, xp=xp))
        multi_signals_2 = MultiSignals(xp_asarray(self._range_2, xp=xp))
...
multi_signals_unpack_data` method.
        ...
        signals = MultiSignals.multi_signals_unpack_data(
            xp_asarray(self._range_1, xp=xp)
        )
        assert list(signals.keys()) == ["0"]
        xp_assert_equal(signals["0"].domain, self._domain_1)
        xp_assert_equal(signals["0"].range, self._range_1)

        signals = MultiSignals.multi_signals_unpack_data(
            xp_asarray(self._range_1, xp=xp), xp_asarray(self._domain_2, xp=xp)
        )
        assert list(signals.keys()) == ["0"]
        xp_assert_equal(signals["0"].domain, self._domain_2)
        xp_assert_equal(signals["0"].range, self._range_1)
        ...
        xp_assert_equal(signals["0"].range, self._range_1)
        xp_assert_equal(signals["1"].range, self._range_1 + 10)
        xp_assert_equal(signals["2"].range, self._range_1 + 20)
        ...
        if is_pandas_installed():
            ...
            xp_assert_equal(signals["a"].range, self._range_1)
            xp_assert_equal(signals["b"].range, self._range_1 + 10)
            xp_assert_equal(signals["c"].range, self._range_1 + 20)
