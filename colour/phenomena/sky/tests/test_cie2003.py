...
from colour.utilities import ignore_numpy_errors, xp_asarray, xp_assert_close
...
sky_luminance_gradation_CIE2003` definition.
        ...
        xp_assert_close(
            sky_luminance_gradation_CIE2003(xp_asarray([0.0], xp=xp), 4.0, -0.70),
            1 + 4.0 * np.exp(-0.70),
        ...
        xp_assert_close(
            sky_luminance_gradation_CIE2003(xp_asarray([np.pi / 2], xp=xp), 4.0, -0.70),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            sky_luminance_gradation_CIE2003(xp_asarray([0.0], xp=xp), 0, -1.0),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            sky_luminance_gradation_CIE2003(
                xp_asarray([np.radians(45)], xp=xp), 0, -1.0
            ),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
sky_luminance_gradation_CIE2003` definition n-dimensional support.
        ...
        Z = xp_asarray([0, np.radians(30), np.radians(60)], xp=xp)
...
sky_scattering_indicatrix_CIE2003` definition.
        ...
        xp_assert_close(
            sky_scattering_indicatrix_CIE2003(
                xp_asarray([np.radians(45)], xp=xp), 0, -1.0, 0
            ),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            sky_scattering_indicatrix_CIE2003(xp_asarray([0.0], xp=xp), 10, -3.0, 0.45),
        ...
        xp_assert_close(
            sky_scattering_indicatrix_CIE2003(
                xp_asarray([np.radians(30)], xp=xp), 10, -3.0, 0.45
            ),
            3.32646285329632,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
sky_scattering_indicatrix_CIE2003` definition n-dimensional support.
        ...
        chi = xp_asarray([0, np.radians(45), np.radians(90)], xp=xp)
...
sky_luminance_distribution_CIE2003` definition.
        ...
        xp_assert_close(
            sky_luminance_distribution_CIE2003(
                5,
                xp_asarray([np.radians(45)], xp=xp),
                xp_asarray([np.radians(90)], xp=xp),
                np.radians(30),
                np.radians(0),
            ),
            1.0,
        ...
        xp_assert_close(
            sky_luminance_distribution_CIE2003(
                1,
                xp_asarray([np.radians(45)], xp=xp),
                xp_asarray([np.radians(180)], xp=xp),
                np.radians(30),
                np.radians(0),
            ),
            0.83258464279644,
...
sky_luminance_distribution_CIE2003` definition n-dimensional support.
        ...
        Z = xp_asarray([0, np.radians(30), np.radians(60)], xp=xp)
        alpha = xp_asarray([0, np.radians(90), np.radians(180)], xp=xp)
...
sky_luminance_distribution_overcast_CIE2003` definition.
        ...
        xp_assert_close(
            sky_luminance_distribution_overcast_CIE2003(xp_asarray([0.0], xp=xp)),
            1.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            sky_luminance_distribution_overcast_CIE2003(xp_asarray([np.pi / 2], xp=xp)),
            1 / 3,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            sky_luminance_distribution_overcast_CIE2003(xp_asarray([np.pi / 4], xp=xp)),
