...
class TestSnellLaw:
    ...
    def test_snell_law(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            snell_law(1.0, 1.5, xp_asarray([30.0], xp=xp)),
            19.4712206345,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            snell_law(1.0, 1.33, xp_asarray([45.0], xp=xp)),
            32.117631278,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            snell_law(1.5, 1.0, xp_asarray([19.47], xp=xp)),
            30.0,
        ...
        xp_assert_close(
            snell_law(1.0, 1.5, xp_asarray([0.0], xp=xp)),
            0.0,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_snell_law(self, xp: ModuleType) -> None:
        ...
        theta_i = xp.tile(xp_asarray(theta_i, xp=xp), (6,))
        theta_t = xp.tile(xp_asarray(theta_t, xp=xp), (6,))
        xp_assert_close(
        ...
        theta_i = xp_reshape(xp_asarray(theta_i, xp=xp), (2, 3), xp=xp)
        theta_t = xp_reshape(xp_asarray(theta_t, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            snell_law(n_1, n_2, theta_i),
            theta_t,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
class TestPolarisedLightMagnitudeElements:
    ...
    def test_polarised_light_magnitude_elements(self, xp: ModuleType) -> None:
        ...
        result = polarised_light_magnitude_elements(
            1.0, 1.5, xp_asarray([0.0], xp=xp), xp_asarray([0.0], xp=xp)
        )
        ...
        result_45 = polarised_light_magnitude_elements(
            1.0, 1.5, xp_asarray([45.0], xp=xp), xp_asarray([30.0], xp=xp)
        )
    ...
    def test_n_dimensional_polarised_light_magnitude_elements(
        ...
        theta_i_array = xp.tile(xp_asarray(theta_i, xp=xp), (6,))
        theta_t_array = xp.tile(xp_asarray(theta_t, xp=xp), (6,))
        m0_array, m1_array, m2_array, m3_array = polarised_light_magnitude_elements(
            n_1, n_2, theta_i_array, theta_t_array
        )
        xp_assert_close(
            m0_array,
            xp.tile(xp_asarray(m0, xp=xp), (6,)),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        xp_assert_close(
            m1_array,
            xp.tile(xp_asarray(m1, xp=xp), (6,)),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        xp_assert_close(
            m2_array,
            xp.tile(xp_asarray(m2, xp=xp), (6,)),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        xp_assert_close(
            m3_array,
            xp.tile(xp_asarray(m3, xp=xp), (6,)),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        theta_i_array = xp_reshape(xp_asarray(theta_i_array, xp=xp), (2, 3), xp=xp)
        theta_t_array = xp_reshape(xp_asarray(theta_t_array, xp=xp), (2, 3), xp=xp)
        m0_array, m1_array, m2_array, m3_array = polarised_light_magnitude_elements(
            n_1, n_2, theta_i_array, theta_t_array
        )
        xp_assert_close(
            m0_array,
            xp.tile(xp_asarray(m0, xp=xp), (6,)).reshape(2, 3),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        xp_assert_close(
            m1_array,
            xp.tile(xp_asarray(m1, xp=xp), (6,)).reshape(2, 3),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        xp_assert_close(
            m2_array,
            xp.tile(xp_asarray(m2, xp=xp), (6,)).reshape(2, 3),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        xp_assert_close(
            m3_array,
            xp.tile(xp_asarray(m3, xp=xp), (6,)).reshape(2, 3),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
class TestPolarisedLightReflectionAmplitude:
    ...
    def test_polarised_light_reflection_amplitude(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            polarised_light_reflection_amplitude(
                1.0,
                1.5,
                xp_asarray([30.0], xp=xp),
                xp_asarray([19.47], xp=xp),
            ),
    ...
    def test_n_dimensional_polarised_light_reflection_amplitude(
        ...
        xp_assert_close(
            r_array,
            xp.tile(xp_asarray(r, xp=xp), (6, 1)),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            r_array,
            xp.tile(xp_asarray(r, xp=xp), (6, 1)).reshape(2, 3, 2),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
class TestPolarisedLightTransmissionAmplitude:
    ...
    def test_n_dimensional_polarised_light_transmission_amplitude(
        ...
        xp_assert_close(
            t_array,
            xp.tile(xp_asarray(t, xp=xp), (6, 1)),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            t_array,
            xp.tile(xp_asarray(t, xp=xp), (6, 1)).reshape(2, 3, 2),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
class TestMatrixTransferTmm:
    ...
    def test_matrix_transfer_tmm(self, xp: ModuleType) -> None:
        ...
        result = matrix_transfer_tmm(
            ...
            t=[250],
            theta=0,
            wavelength=xp_asarray([550], xp=xp),
        )
    ...
    def test_matrix_transfer_tmm_multilayer(self, xp: ModuleType) -> None:
        ...
        result = matrix_transfer_tmm(
            n=[1.0, 1.5, 2.0, 1.5],
            t=[250, 150],
            theta=xp_asarray([0.0], xp=xp),
            wavelength=xp_asarray([550.0], xp=xp),
        )
    ...
    def test_matrix_transfer_tmm_multiple_wavelengths(self, xp: ModuleType) -> None:
        ...
        wavelengths = xp_asarray([400.0, 500.0, 600.0], xp=xp)
        result = matrix_transfer_tmm(
            n=[1.0, 1.5, 1.0],
            t=[250],
            theta=xp_asarray([0.0], xp=xp),
    ...
    def test_matrix_transfer_tmm_complex_n(self, xp: ModuleType) -> None:
        ...
        # n has shape (media_count, wavelengths_count)
        assert np.iscomplex(np.asarray(result.n)[1, 0])
        xp_assert_close(result.n[1, 0], n_absorbing, atol=TOLERANCE_ABSOLUTE_TESTS)

        # Transfer matrices should be complex
        assert np.iscomplexobj(np.asarray(result.M_s))
        assert np.iscomplexobj(np.asarray(result.M_p))
    ...
    def test_matrix_transfer_tmm_oblique_incidence(self, xp: ModuleType) -> None:
        ...
        result = matrix_transfer_tmm(
            n=[1.0, 1.5, 1.0],
            t=[250],
            theta=xp_asarray([theta_i], xp=xp),
            wavelength=xp_asarray([550.0], xp=xp),
        )
        ...
        assert np.asarray(result.theta)[0, 1] != theta_i  # Should be refracted

        # s and p matrices should differ at oblique incidence
        assert not np.allclose(
            np.asarray(result.M_s),
            np.asarray(result.M_p),
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_matrix_transfer_tmm_energy_consistency(self, xp: ModuleType) -> None:
        ...
        M_s = np.asarray(result.M_s)
        r_s = M_s[0, 0, 0, 1, 0] / M_s[0, 0, 0, 0, 0]
        R_s = np.abs(r_s) ** 2

        t_s = 1.0 / M_s[0, 0, 0, 0, 0]
        theta_i_rad = np.radians(0.0)
        theta_f_rad = np.radians(np.asarray(result.theta)[0, -1])

        # Extract incident and substrate from result.n
        n_incident = np.asarray(result.n)[0, 0]
        n_substrate = np.asarray(result.n)[-1, 0]
    ...
    def test_n_dimensional_matrix_transfer_tmm(self, xp: ModuleType) -> None:
        ...
        assert np.asarray(result_array.M_p).shape == (6, 1, 1, 2, 2)

        # theta shapes: result has (1, 3), result_array has (1, 3)
        assert np.asarray(result_array.theta).shape == result.theta.shape
        # n shapes: result has (3, 1), result_array has (3, 6)
        # For constant n, all wavelength columns should match
        assert np.asarray(result_array.n).shape == (3, 6)
        assert result.n.shape == (3, 1)
        xp_assert_close(
            result_array.n[:, 0],
            result.n[:, 0],
