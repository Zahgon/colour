...
def ellipse_coefficients_general_form(coefficients: ArrayLike) -> NDArrayFloat:
    ...
    return xp_asarray([a, b, c, d, e, f], xp=xp)
...
def ellipse_coefficients_canonical_form(
    ...
    return xp_asarray([x_c, y_c, a_a, a_b, theta], xp=xp)
...
def ellipse_fitting_Halir1998(a: ArrayLike) -> NDArrayFloat:
    ...
    xp = array_namespace(D1)

    D1_T = xp.matrix_transpose(D1)
    D2_T = xp.matrix_transpose(D2)
    ...
    T = -xp.matmul(xp.linalg.inv(S3), xp.matrix_transpose(S2))

    # Reduced scatter matrix.
    M = S1 + xp.matmul(S2, T)
    M = xp.stack([M[2, :] / 2, -M[1, :], M[0, :] / 2])

    _w, v = xp.linalg.eig(M)
    v = xp.real(v)
