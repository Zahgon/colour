...
class TestChromaticAdaptationZhai2018:
    ...
    def test_chromatic_adaptation_Zhai2018(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            chromatic_adaptation_Zhai2018(
                XYZ_b=xp_asarray([48.900, 43.620, 6.250], xp=xp),
                XYZ_wb=xp_asarray([109.850, 100, 35.585], xp=xp),
                XYZ_wd=xp_asarray([95.047, 100, 108.883], xp=xp),
                D_b=0.9407,
                D_d=0.9800,
                XYZ_wo=xp_asarray([100, 100, 100], xp=xp),
            ),
        ...
        xp_assert_close(
            chromatic_adaptation_Zhai2018(
                XYZ_b=xp_asarray([52.034, 58.824, 23.703], xp=xp),
                XYZ_wb=xp_asarray([92.288, 100, 38.775], xp=xp),
                XYZ_wd=xp_asarray([105.432, 100, 137.392], xp=xp),
                D_b=0.6709,
                D_d=0.5331,
                XYZ_wo=xp_asarray([97.079, 100, 141.798], xp=xp),
            ),
    ...
    def test_n_dimensional_chromatic_adaptation_Zhai2018(self, xp: ModuleType) -> None:
        ...
        XYZ_b = xp.tile(xp_asarray(XYZ_b, xp=xp), (6, 1))
        ...
        XYZ_wb = xp.tile(xp_asarray(XYZ_wb, xp=xp), (6, 1))
        XYZ_wd = xp.tile(xp_asarray(XYZ_wd, xp=xp), (6, 1))
        D_b = xp.tile(xp_asarray(D_b, xp=xp), (6, 1))
        D_d = xp.tile(xp_asarray(D_d, xp=xp), (6, 1))
        xp_assert_close(
            chromatic_adaptation_Zhai2018(XYZ_b, XYZ_wb, XYZ_wd, D_b, D_d),
            XYZ_d,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        XYZ_b = xp_reshape(xp_asarray(XYZ_b, xp=xp), (2, 3, 3), xp=xp)
        XYZ_wb = xp_reshape(xp_asarray(XYZ_wb, xp=xp), (2, 3, 3), xp=xp)
        XYZ_wd = xp_reshape(xp_asarray(XYZ_wd, xp=xp), (2, 3, 3), xp=xp)
        D_b = xp_reshape(xp_asarray(D_b, xp=xp), (2, 3, 1), xp=xp)
        D_d = xp_reshape(xp_asarray(D_d, xp=xp), (2, 3, 1), xp=xp)
    ...
    def test_domain_range_scale_chromatic_adaptation_Zhai2018(
        ...
        XYZ_wb = xp_asarray([109.850, 100, 35.585], xp=xp)
        XYZ_wd = xp_asarray([95.047, 100, 108.883], xp=xp)
        XYZ_d = np.asarray(chromatic_adaptation_Zhai2018(XYZ_b, XYZ_wb, XYZ_wd))
