...
def chromatic_adaptation_Zhai2018(
    ...
    *Zhai and Luo (2018)* chromatic adaptation model.
    ...
    XYZ_wb = xp_asarray(XYZ_wb, xp=xp, like=XYZ_b)
    XYZ_wd = xp_asarray(XYZ_wd, xp=xp, like=XYZ_b)
    XYZ_wo = xp_asarray(XYZ_wo, xp=xp, like=XYZ_b)
    D_b = xp_asarray(D_b, xp=xp, like=XYZ_b)
    D_d = xp_asarray(D_d, xp=xp, like=XYZ_b)
    ...
    M = xp_asarray(CHROMATIC_ADAPTATION_TRANSFORMS[transform], xp=xp, like=XYZ_b)
