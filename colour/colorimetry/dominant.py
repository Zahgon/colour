...
def closest_spectral_locus_wavelength(
    ...
    xy_n = xp_asarray(xp_resize(xy_n, xy.shape, xp=xp), xp=xp, like=xy)
    ...
    xy_s = xp_asarray(xy_s, xp=xp, like=xy)
    ...
    _xy_wl = as_ndarray(xy_wl)
    _xy_s = as_ndarray(xy_s)

    i_wl = xp.argmin(
        xp_asarray(
            scipy.spatial.distance.cdist(_xy_wl, _xy_s),
            xp=xp,
            like=xy_wl,
        ),
        axis=-1,
...
def dominant_wavelength(
    ...
    *Complementary dominant wavelength* is returned if the first intersection
    ...
    xy_s = XYZ_to_xy(cmfs.values)
    wavelengths = xp_asarray(cmfs.wavelengths, xp=xp, like=xy)
    ...
    squeeze_axes = tuple(i for i, s in enumerate(xy_wl.shape) if s == 1)
    if squeeze_axes:
        xy_wl = xp.squeeze(xy_wl, axis=squeeze_axes)
        xy_cwl = xp.squeeze(xy_cwl, axis=squeeze_axes)
