...
class TestLightnessGlasser1958:
    ...
    def test_lightness_Glasser1958(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            lightness_Glasser1958(xp_asarray([12.19722535], xp=xp)),
            39.83512646492521,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Glasser1958(xp_asarray([23.04276781], xp=xp)),
            53.585946877480623,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Glasser1958(xp_asarray([6.15720079], xp=xp)),
            27.972867038082629,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_domain_range_scale_lightness_Glasser1958(self) -> None:
        ...
        L = np.asarray(lightness_Glasser1958(12.19722535))
...
class TestLightnessWyszecki1963:
    ...
    def test_lightness_Wyszecki1963(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            lightness_Wyszecki1963(xp_asarray([12.19722535], xp=xp)),
            40.547574599570197,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Wyszecki1963(xp_asarray([23.04276781], xp=xp)),
            54.140714588256841,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Wyszecki1963(xp_asarray([6.15720079], xp=xp)),
            28.821339499883976,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_lightness_Wyszecki1963(self, xp: ModuleType) -> None:
        ...
        Y = xp.tile(xp_asarray(Y, xp=xp), (6,))
        W = xp.tile(xp_asarray(W, xp=xp), (6,))
        xp_assert_close(lightness_Wyszecki1963(Y), W, atol=TOLERANCE_ABSOLUTE_TESTS)

        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3), xp=xp)
        W = xp_reshape(xp_asarray(W, xp=xp), (2, 3), xp=xp)
        xp_assert_close(lightness_Wyszecki1963(Y), W, atol=TOLERANCE_ABSOLUTE_TESTS)

        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3, 1), xp=xp)
        W = xp_reshape(xp_asarray(W, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(lightness_Wyszecki1963(Y), W, atol=TOLERANCE_ABSOLUTE_TESTS)
    ...
    def test_domain_range_scale_lightness_Wyszecki1963(self) -> None:
        ...
        W = np.asarray(lightness_Wyszecki1963(12.19722535))
...
intermediate_lightness_function_CIE1976` definition.
        ...
        xp_assert_close(
            intermediate_lightness_function_CIE1976(xp_asarray([12.19722535], xp=xp)),
            0.495929964178047,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            intermediate_lightness_function_CIE1976(xp_asarray([23.04276781], xp=xp)),
            0.613072093530391,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            intermediate_lightness_function_CIE1976(xp_asarray([6.15720079], xp=xp)),
            0.394876333449113,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
intermediate_lightness_function_CIE1976` definition n-dimensional arrays
        ...
        Y = xp.tile(xp_asarray(Y, xp=xp), (6,))
        f_Y_Y_n = xp.tile(xp_asarray(f_Y_Y_n, xp=xp), (6,))
        xp_assert_close(
        ...
        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3), xp=xp)
        f_Y_Y_n = xp_reshape(xp_asarray(f_Y_Y_n, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            intermediate_lightness_function_CIE1976(Y),
            f_Y_Y_n,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        ...
        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3, 1), xp=xp)
        f_Y_Y_n = xp_reshape(xp_asarray(f_Y_Y_n, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(
            intermediate_lightness_function_CIE1976(Y),
            f_Y_Y_n,
            atol=TOLERANCE_ABSOLUTE_TESTS,
...
intermediate_lightness_function_CIE1976` definition domain and range scale
        ...
        f_Y_Y_n = np.asarray(intermediate_lightness_function_CIE1976(12.19722535, 100))
...
class TestLightnessCIE1976:
    ...
    def test_lightness_CIE1976(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            lightness_CIE1976(xp_asarray([12.19722535], xp=xp)),
            41.527875844653451,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_CIE1976(xp_asarray([23.04276781], xp=xp)),
            55.116362849525402,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_CIE1976(xp_asarray([6.15720079], xp=xp)),
            29.805654680097106,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_CIE1976(xp_asarray([12.19722535], xp=xp), 50),
            56.480581732417676,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_CIE1976(xp_asarray([12.19722535], xp=xp), 75),
            47.317620274162735,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_CIE1976(xp_asarray([12.19722535], xp=xp), 95),
            42.519930728120940,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_lightness_CIE1976(self, xp: ModuleType) -> None:
        ...
        Y = xp.tile(xp_asarray(Y, xp=xp), (6,))
        L_star = xp.tile(xp_asarray(L_star, xp=xp), (6,))
        xp_assert_close(lightness_CIE1976(Y), L_star, atol=TOLERANCE_ABSOLUTE_TESTS)

        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3), xp=xp)
        L_star = xp_reshape(xp_asarray(L_star, xp=xp), (2, 3), xp=xp)
        xp_assert_close(lightness_CIE1976(Y), L_star, atol=TOLERANCE_ABSOLUTE_TESTS)

        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3, 1), xp=xp)
        L_star = xp_reshape(xp_asarray(L_star, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(lightness_CIE1976(Y), L_star, atol=TOLERANCE_ABSOLUTE_TESTS)
    ...
    def test_domain_range_scale_lightness_CIE1976(self) -> None:
        ...
        L_star = np.asarray(lightness_CIE1976(12.19722535, 100))
...
class TestLightnessFairchild2010:
    ...
    def test_lightness_Fairchild2010(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            lightness_Fairchild2010(xp_asarray([12.19722535 / 100], xp=xp)),
            31.996390226262736,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2010(xp_asarray([23.04276781 / 100], xp=xp)),
            60.203153682783302,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2010(xp_asarray([6.15720079 / 100], xp=xp)),
            11.836517240976489,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2010(xp_asarray([12.19722535 / 100], xp=xp), 2.75),
            24.424283249379986,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2010(xp_asarray([1008.0], xp=xp)),
            100.019986327374240,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2010(xp_asarray([100800.0], xp=xp)),
            100.019999997090270,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_n_dimensional_lightness_Fairchild2010(self, xp: ModuleType) -> None:
        ...
        Y = xp.tile(xp_asarray(Y, xp=xp), (6,))
        L_hdr = xp.tile(xp_asarray(L_hdr, xp=xp), (6,))
        xp_assert_close(
            lightness_Fairchild2010(Y), L_hdr, atol=TOLERANCE_ABSOLUTE_TESTS
        )

        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3), xp=xp)
        L_hdr = xp_reshape(xp_asarray(L_hdr, xp=xp), (2, 3), xp=xp)
        xp_assert_close(
            lightness_Fairchild2010(Y), L_hdr, atol=TOLERANCE_ABSOLUTE_TESTS
        )

        Y = xp_reshape(xp_asarray(Y, xp=xp), (2, 3, 1), xp=xp)
        L_hdr = xp_reshape(xp_asarray(L_hdr, xp=xp), (2, 3, 1), xp=xp)
        xp_assert_close(
            lightness_Fairchild2010(Y), L_hdr, atol=TOLERANCE_ABSOLUTE_TESTS
        )
    ...
    def test_domain_range_scale_lightness_Fairchild2010(self) -> None:
        ...
        L_hdr = np.asarray(lightness_Fairchild2010(12.19722535 / 100))
...
class TestLightnessFairchild2011:
    ...
    def test_lightness_Fairchild2011(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            lightness_Fairchild2011(xp_asarray([12.19722535 / 100], xp=xp)),
            51.852958445912506,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2011(xp_asarray([23.04276781 / 100], xp=xp)),
            65.275207956353853,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2011(xp_asarray([6.15720079 / 100], xp=xp)),
            39.818935510715917,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2011(xp_asarray([12.19722535 / 100], xp=xp), 2.75),
            0.13268968410139345,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2011(xp_asarray([1008.0], xp=xp)),
            234.72925682,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Fairchild2011(xp_asarray([100800.0], xp=xp)),
            245.5705978,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
    ...
    def test_domain_range_scale_lightness_Fairchild2011(self) -> None:
        ...
        L_hdr = np.asarray(lightness_Fairchild2011(12.19722535 / 100))
...
class TestLightnessAbebe2017:
    ...
    def test_lightness_Abebe2017(self, xp: ModuleType) -> None:
        ...
        xp_assert_close(
            lightness_Abebe2017(xp_asarray([12.19722535], xp=xp)),
            0.486955571109229,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Abebe2017(xp_asarray([12.19722535], xp=xp), method="Stevens"),
            0.474544792145434,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Abebe2017(xp_asarray([12.19722535], xp=xp), 1000),
            0.286847428534793,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )

        xp_assert_close(
            lightness_Abebe2017(xp_asarray([12.19722535], xp=xp), 4000),
            0.192145492588158,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
        ...
        xp_assert_close(
            lightness_Abebe2017(
                xp_asarray([12.19722535], xp=xp), 4000, method="Stevens"
            ),
            0.170365211220992,
            atol=TOLERANCE_ABSOLUTE_TESTS,
        )
...
class TestLightness:
    ...
    def test_domain_range_scale_lightness(self) -> None:
        ...
        v = [np.asarray(lightness(12.19722535, method, Y_n=100)) for method in m]
