...
closest_spectral_locus_wavelength` definition.
        ...
        xy = xp_asarray([0.37605506, 0.24452225], xp=xp)
        i_wl, xy_wl = closest_spectral_locus_wavelength(xy, xy_n, self._xy_s)
...
closest_spectral_locus_wavelength` definition n-dimensional arrays support.
        ...
        i_wl, xy_wl = closest_spectral_locus_wavelength(xy, xy_n, self._xy_s)
        i_wl_r = xp.tile(xp_asarray(i_wl_r, xp=xp), (6,))
        xy_wl_r = xp.tile(xp_asarray(xy_wl_r, xp=xp), (6, 1))
        xp_assert_close(i_wl, i_wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        xp_assert_close(xy_wl, xy_wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        ...
        i_wl, xy_wl = closest_spectral_locus_wavelength(xy, xy_n, self._xy_s)
        i_wl_r = xp_reshape(xp_asarray(i_wl_r, xp=xp), (2, 3), xp=xp)
        xy_wl_r = xp_reshape(xp_asarray(xy_wl_r, xp=xp), (2, 3, 2), xp=xp)
        xp_assert_close(i_wl, i_wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        xp_assert_close(xy_wl, xy_wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestDominantWavelength:
    ...
    def test_n_dimensional_dominant_wavelength(self, xp: ModuleType) -> None:
        ...
        xy_n = xp.tile(xp_asarray(xy_n, xp=xp), (6, 1))
        wl, xy_wl, xy_cwl = dominant_wavelength(xy, xy_n)
        wl_r = xp.tile(xp_asarray(wl_r, xp=xp), (6,))
        xy_wl_r = xp.tile(xp_asarray(xy_wl_r, xp=xp), (6, 1))
        xy_cwl_r = xp.tile(xp_asarray(xy_cwl_r, xp=xp), (6, 1))
        xp_assert_close(wl, wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        xp_assert_close(xy_wl, xy_wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        xp_assert_close(xy_cwl, xy_cwl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        ...
        xy_n = xp_reshape(xp_asarray(xy_n, xp=xp), (2, 3, 2), xp=xp)
        wl, xy_wl, xy_cwl = dominant_wavelength(xy, xy_n)
        wl_r = xp_reshape(xp_asarray(wl_r, xp=xp), (2, 3), xp=xp)
        xy_wl_r = xp_reshape(xp_asarray(xy_wl_r, xp=xp), (2, 3, 2), xp=xp)
        xy_cwl_r = xp_reshape(xp_asarray(xy_cwl_r, xp=xp), (2, 3, 2), xp=xp)
        xp_assert_close(wl, wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        xp_assert_close(xy_wl, xy_wl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
        xp_assert_close(xy_cwl, xy_cwl_r, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestExcitationPurity:
    ...
    def test_n_dimensional_excitation_purity(self, xp: ModuleType) -> None:
        ...
        xy = xp_asarray([0.54369557, 0.32107944], xp=xp)
        xy_n = self._xy_D65
        P_e = np.asarray(excitation_purity(xy, xy_n))

        xy = xp.tile(xp_asarray(xy, xp=xp), (6, 1))
        xy_n = xp.tile(xp_asarray(xy_n, xp=xp), (6, 1))
        P_e = xp.tile(xp_asarray(P_e, xp=xp), (6,))
        xp_assert_close(excitation_purity(xy, xy_n), P_e, atol=TOLERANCE_ABSOLUTE_TESTS)

        xy = xp_reshape(xp_asarray(xy, xp=xp), (2, 3, 2), xp=xp)
        xy_n = xp_reshape(xp_asarray(xy_n, xp=xp), (2, 3, 2), xp=xp)
        P_e = xp_reshape(xp_asarray(P_e, xp=xp), (2, 3), xp=xp)
        xp_assert_close(excitation_purity(xy, xy_n), P_e, atol=TOLERANCE_ABSOLUTE_TESTS)
...
class TestColorimetricPurity:
    ...
    def test_n_dimensional_colorimetric_purity(self, xp: ModuleType) -> None:
        ...
        xy = xp_asarray([0.54369557, 0.32107944], xp=xp)
        xy_n = self._xy_D65
        P_e = np.asarray(colorimetric_purity(xy, xy_n))
