...
class TestMsdsZeros:
    ...
    def test_msds_zeros(self) -> None:
        ...
        np.testing.assert_equal(np.asarray(msds[360]), np.array([0, 0, 0]))

        np.testing.assert_equal(np.asarray(msds[555]), np.array([0, 0, 0]))

        np.testing.assert_equal(np.asarray(msds[780]), np.array([0, 0, 0]))
...
class TestMsdsOnes:
    ...
    def test_msds_ones(self) -> None:
        ...
        np.testing.assert_equal(np.asarray(msds[360]), np.array([1, 1, 1]))

        np.testing.assert_equal(np.asarray(msds[555]), np.array([1, 1, 1]))

        np.testing.assert_equal(np.asarray(msds[780]), np.array([1, 1, 1]))
...
class TestSdMultiLedsOhno2005:
    ...
    def test_sd_multi_leds_Ohno2005(self, xp: ModuleType) -> None:
        ...
        sd = sd_multi_leds_Ohno2005(
            xp_asarray([457, 530, 615], xp=xp),
            xp_asarray([20, 30, 20], xp=xp),
            xp_asarray([0.731, 1.000, 1.660], xp=xp),
        )
...
class TestSdMultiLeds:
    ...
    def test_sd_multi_leds(self, xp: ModuleType) -> None:
        ...
        sd = sd_multi_leds(
            xp_asarray([457, 530, 615], xp=xp),
            half_spectral_widths=xp_asarray([20, 30, 20], xp=xp),
            peak_power_ratios=xp_asarray([0.731, 1.000, 1.660], xp=xp),
        )
        xp_assert_close(sd[500], 0.129513248576116, atol=TOLERANCE_ABSOLUTE_TESTS)
